...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Welcome = __webpack_require__(127);
	
	var _Welcome2 = _interopRequireDefault(_Welcome);
	
	var _GameEnd = __webpack_require__(129);
	
	var _GameEnd2 = _interopRequireDefault(_GameEnd);
	
	var _screenfullMin = __webpack_require__(131);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Lane = __webpack_require__(114);
	
	var _Lane2 = _interopRequireDefault(_Lane);
	
	var _DeathParticle = __webpack_require__(115);
	
	var _DeathParticle2 = _interopRequireDefault(_DeathParticle);
	
	var _Collision = __webpack_require__(116);
	...
	var _Countdown = __webpack_require__(118);
	
	var _Countdown2 = _interopRequireDefault(_Countdown);
	
	var _Hazard = __webpack_require__(119);
	
	var _Hazard2 = _interopRequireDefault(_Hazard);
	
	var _GameHUD = __webpack_require__(120);
	...
	var _class = function (_Phaser$State) {
	  ...
	  (0, _createClass3.default)(_class, [{
	    ...
	    value: function create() {
	      ...
	      this.game.sounds.playSoundEffect("xlr8_running", true, 0.5);
	
	      this.gameContainer = this.game.make.group();
	      this.background = this.game.add.tileSprite(0, 0, this.game.width, this.game.height, "background");
	
	      // create container to house background scene objects
	      this.sceneContainer = this.game.make.group();
	
	      // house lane positions for sapwns / movement
	      this.lanePositions = game.data.lane_positions;
	
	      // how wide is a lane base on screen size
	      var step = this.game.height / 5;
	      // start 1 step in
	      var i = step + step / 2;
	      // create 3 lanes
	      var lane = new _Lane2.default(this.game, 0, i, step);
	      this.sceneContainer.addChild(lane);
	      this.lanePositions.push(i);
	
	      // setup hero
	      this.hero = new _Hero2.default(this.game, 400, this.game.height / 2);
	      this.hero.scale.set(1.2);
	      this.hero.lane = 1;
	      this.hero.targetLane = 1;
	      this.hero.position.set(350, this.lanePositions[this.hero.lane]);
	
	      this.hud = new _GameHUD2.default(this.game, { score: true });
	      ...
	      // game settings
	      this.gameSpeed = this.game.data.start_speed;
	      this.gameSpeedIncrease = this.game.data.speed_increase;
	    ...
	    value: function update() {
	      ...
	      if (!this.gameOver) {
	        ...
	        // calculate distance travelled based on time
	        ...
	        switch (this.state) {
	          case -1:
	            ...
	            this.background.tilePosition.x -= distStep;
	            break;
	          case 0:
	            this.background.tilePosition.x -= distStep;
	            break;
	          case 1:
	            ...
	            this.gameSpeed += this.timeStamp.timeStep * (this.gameSpeedIncrease / 1000);
	            break;
	        }
	      } else {
	        if (this.game.time.now - this.timeEnd > 2000) this.game.state.start("GameEnd", true, false, Math.round(this.distanceTravelled / 100));
	      }
	
	      this.hud.setScoreDisplay(Math.round(this.distanceTravelled / 100));
	    ...
	    key: 'updateHero',
	    value: function updateHero(distStep) {
	      // calculate distance hero is from desired lane
	      var dy = this.hero.y - this.lanePositions[this.hero.targetLane];
	      ...
	      if (Math.abs(dy) < distStep) {
	        // just put the hero in place
	        this.hero.y = this.lanePositions[this.hero.targetLane];
	        this.hero.lane = this.hero.targetLane;
	      } else {
	        // other wise move hero towards position
	        if (!this.game.sounds.isSoundPlaying("whoosh")) this.game.sounds.playSoundEffect("whoosh", false, 1.5);
	        var rotation = Math.atan2(0, dy);
	        this.hero.y -= Math.cos(rotation) * laneStep;
	      }
	    }
	  }, {
	    ...
	    value: function updateHazards(distStep) {
	      ...
	      for (i in this.hazards) {
	        if (this.hazards[i].x > this.game.width + 250) garbage.push(this.hazards[i]);else {
	          // move by distance
	          this.hazards[i].x -= distStep;
	
	          // calculate distance from hero
	          var d = Math.sqrt((this.hazards[i].x - this.hero.x) * (this.hazards[i].x - this.hero.x) + (this.hazards[i].y - this.hero.y) * (this.hazards[i].y - this.hero.y));
	
	          // if smaller than hero radius - we hit hero
	          if (d < 90) {
	            this.killHero(i, this.hazards[i].type);
	          }
	        }
	      }
	    ...
	    key: 'activateKeyboardControls',
	    value: function activateKeyboardControls(active) {
	      if (active === true) {
	        this.keyLeft.onDown.add(this.handleUserInput, this);
	        this.keyRight.onDown.add(this.handleUserInput, this);
	      } else {
	        this.keyLeft.onDown.remove(this.handleUserInput, this);
	        this.keyRight.onDown.remove(this.handleUserInput, this);
	      }
	    }
	  }, {
	    key: 'spawnHazard',
	    value: function spawnHazard() {
	      if (this.distanceTravelled < 2500) {
	        this.createHazard(Math.floor(Math.random() * 3));
	      } else {
	        var lanes = [0, 1, 2];
	        //Pick how many
	        var count = Math.ceil(Math.random() * 2);
	        //Pick which lane
	        while (lanes.length > count) {
	          lanes.splice(Math.floor(Math.random() * 3), 1);
	        }for (var i in lanes) {
	          this.createHazard(lanes[i]);
	        }
	      }
	    }
	  }, {
	    ...
	    value: function createHazard(lane) {
	      ...
	      hazard.position.set(this.game.width + 200, this.lanePositions[lane]);
	      this.hazardContainer.addChild(hazard);
	      this.hazards.push(hazard);
	    }
	  }, {
	    key: 'killHero',
	    value: function killHero(i, type) {
	      this.gameOver = true;
	      this.game.sounds.stopSound("xlr8_running");
	      //this.activateKeyboardControls(false);
	
	      this.gameContainer.addChild(new _DeathParticle2.default(this.game, this.hero.x, this.hazards[i].y));
	      this.gameContainer.addChild(new _Collision2.default(this.game, this.hazards[i].x, this.hazards[i].y, type));
	
	      this.hero.destroy();
	      this.hazards[i].destroy();
	
	      this.game.sounds.playSoundEffect("hazard", false, 1);
	    ...
	    value: function handleCountdownComplete() {
	      ...
	      this.gamename.position.set(this.game.width / 2 - this.gamename.width / 2, this.game.height / 2 - this.gamename.height / 2);
	    }
	  }, {
	    key: 'handlePause',
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _class = function (_Phaser$Sprite) {
	  ...
	  function _class(game, x, y, asset) {
	    ...
	    _this.costume.anchor.set(0.75, 0.75);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _class = function (_Phaser$Sprite) {
	  ...
	  function _class(game, x, y, width) {
	    (0, _classCallCheck3.default)(this, _class);
	    return (0, _possibleConstructorReturn3.default)(this, (_class.__proto__ || (0, _getPrototypeOf2.default)(_class)).call(this, game, x, y));
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _class = function (_Phaser$Sprite) {
	  ...
	  function _class(game, x, y) {
	    ...
	    _this.costume = _this.game.make.sprite(0, 0, "hazard_collision_cloud");
	    _this.costume.animations.add("run");
	    _this.costume.animations.play("run", 24, false);
	    _this.costume.anchor.set(0.5, 0.75);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _class = function (_Phaser$Sprite) {
	  ...
	  function _class(game, x, y, type) {
	    ...
	    _this.costume = _this.game.make.sprite(0, 0, "hazard_0" + _this.type + "_collision");
	    _this.costume.animations.add("run");
	    _this.costume.animations.play("run", 24, false);
	    _this.costume.anchor.set(0.5, 0.75);
	    _this.addChild(_this.costume);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _class = function (_Phaser$Sprite) {
	  ...
	  function _class(game, x, y, asset) {
	    ...
	    _this.type = Math.ceil(Math.random() * 3);
	
	    _this.game = game;
	    _this.costume = _this.game.make.sprite(0, 0, "hazard0" + _this.type);
	    _this.costume.anchor.set(0.5, 0.75);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ClockDisplay = __webpack_require__(121);
	
	var _ClockDisplay2 = _interopRequireDefault(_ClockDisplay);
	
	var _ScoreDisplay = __webpack_require__(122);
	
	var _ScoreDisplay2 = _interopRequireDefault(_ScoreDisplay);
	
	var _BtnPause = __webpack_require__(123);
	
	var _BtnPause2 = _interopRequireDefault(_BtnPause);
	
	var _InstructionArrowsUpDown = __webpack_require__(124);
	
	var _InstructionArrowsUpDown2 = _interopRequireDefault(_InstructionArrowsUpDown);
	
	var _InstructionSwipe = __webpack_require__(125);
	
	var _InstructionSwipe2 = _interopRequireDefault(_InstructionSwipe);
	
	var _BtnSoundToggle = __webpack_require__(126);
	...
	var _class = function (_Phaser$Sprite) {
	    ...
	    function _class(game, args) {
	        ...
	        _this.instruction = _this.game.isTouch ? new _InstructionSwipe2.default(_this.game) : new _InstructionArrowsUpDown2.default(_this.game);
	...
	var _class = function (_Phaser$Sprite) {
	  ...
	  function _class(game, x, y) {
	    ...
	    if (!_this.game.alreadyPlayed) {
	      _this.arrowUp = _this.game.make.sprite(0, 0, "instruction_arrow_up");
	      _this.arrowDown = _this.game.make.sprite(0, 0, "instruction_arrow_down");
	
	      _this.arrowUp.anchor.set(0.5, 1);
	      _this.arrowDown.anchor.set(0.5, 0);
	      _this.arrowUp.scale.set(0.5);
	      _this.arrowDown.scale.set(0.5);
	
	      _this.arrowUp.position.set(0, -10);
	      _this.arrowDown.position.set(0, 10);
	      _this.game.alreadyPlayed = true;
	      _this.anchor.set(0.5);
	      _this.position.set(_this.game.width / 2, _this.game.height / 2);
	      _this.addChild(_this.arrowUp);
	      _this.addChild(_this.arrowDown);
	      _this.tweenArrows();
	    } else {
	      _this.visible = false;
	    }
	  ...
	  (0, _createClass3.default)(_class, [{
	    ...
	    value: function tweenArrows() {
	      this.tweenUpIn = this.game.add.tween(this.arrowUp.scale).to({ x: 0.25, y: 0.25 }, 250, "Quart.easeOut", false, 250);
	      this.tweenUpOut = this.game.add.tween(this.arrowUp.scale).to({ x: 0.5, y: 0.5 }, 250, "Quart.easeOut");
	      this.tweenDownIn = this.game.add.tween(this.arrowDown.scale).to({ x: 0.25, y: 0.25 }, 250, "Quart.easeOut", false, 250);
	      this.tweenDownOut = this.game.add.tween(this.arrowDown.scale).to({ x: 0.5, y: 0.5 }, 250, "Quart.easeOut");
	      this.tweenOut = this.game.add.tween(this.scale).to({ x: 0, y: 0 }, 250, "Quart.easeOut", false, 250);
	
	      this.tweenUpIn.chain(this.tweenUpOut);
	      this.tweenUpOut.chain(this.tweenDownIn);
	      this.tweenDownIn.chain(this.tweenDownOut);
	      this.tweenDownOut.chain(this.tweenOut);
	
	      this.tweenUpIn.start();
	    }
	  }]);
	  return _class;
	}(Phaser.Sprite);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _BtnPlay = __webpack_require__(128);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _BtnReplay = __webpack_require__(130);
	...
	var _class = function (_Phaser$State) {
	    ...
	    (0, _createClass3.default)(_class, [{
	        ...
	        value: function renderDefault() {
	            ...
	            this.endIcon = this.game.add.sprite(this.game.width * 0.33, this.game.height * 0.29, "icon_timeup");
	        ...
	        value: function renderPromotedGames() {
	            ...
	            this.btnReplay.position.set(this.game.width / 2, this.game.height * 0.28);
	            this.game.add.existing(this.btnReplay);
