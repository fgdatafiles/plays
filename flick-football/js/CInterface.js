function CInterface(iPlayerTeam, iOpponentTeam) {
    ...
    this.createEndMatchText = function (iGoalPlayer, iOpponentPlayer, bWin, iScore, bEnd, bMultiplayer, bDraw) {
        ...
        createjs.Tween.get(oContainer).to({y: TWEEN_END_MACTH_Y}, 1250, createjs.Ease.elasticOut).call(function () {
            createjs.Tween.get(oContainer).to({scaleX: 0, scaleY: 0}, 500, createjs.Ease.quartIn).call(function () {
					if (bDraw){
						//s_oInterface.createDrawPanel(iGoalPlayer, iOpponentPlayer, iScore, bMultiplayer);
						s_oInterface.createDrawPanel(iGoalPlayer, iOpponentPlayer, iScore, bEnd, bMultiplayer);
					}
					else{
						if (bWin) {
							s_oInterface.createWinPanel(iGoalPlayer, iOpponentPlayer, iScore, bEnd, bMultiplayer);
						} else {
							s_oInterface.createLosePanel(iGoalPlayer, iOpponentPlayer, iScore, bMultiplayer);
						}
					}
    ...
    this.createLosePanel = function (iGoalPlayer, iOpponentPlayer, iScore, bMultiplayer) {
        _oLosePanel = new CLosePanel(s_oSpriteLibrary.getSprite("msg_box"), bMultiplayer);
        _oLosePanel.show(iGoalPlayer, iOpponentPlayer, _iPlayerTeam, _iOpponentTeam, iScore);
    };

	this.createDrawPanel = function (iGoalPlayer, iOpponentPlayer, iScore, bMultiplayer) {
        _oDrawPanel = new CDrawPanel(s_oSpriteLibrary.getSprite("msg_box"), bMultiplayer);
        _oDrawPanel.show(iGoalPlayer, iOpponentPlayer, _iPlayerTeam, _iOpponentTeam, iScore);
    };

    this.createWinPanel = function (iGoalPlayer, iOpponentPlayer, iScore, bEnd, bMultiplayer) {
        _oWinPanel = new CWinPanel(s_oSpriteLibrary.getSprite("msg_box"), bEnd, bMultiplayer);
