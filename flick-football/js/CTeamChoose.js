function CTeamChoose(bMultiplayerMode) {
    var _pStartPosAudio;
    var _pStartPosExit;
	...
	var _bPlayerTwoSelected = false;
	var _bPlayerOneSelected = false;
	var _bMultiplayer;
    var _oBg;
    var _oMsgBox;
    var _oButContinue;
    ...
    this._init = function () {
		_bMultiplayer = bMultiplayerMode;
        ...
        for (var i = 0; i < TOT_TEAM; i++) {
			var oSpriteFlagSmall = s_oSpriteLibrary.getSprite("flags_0" + i);
		
			this._createBigFlag(i, iOffsetX, iOffsetY, oSpriteFlagSmall, _oContainer);
			//if (i >= 4){
		...
		oSelectTeamText = new createjs.Text("PLAYER 1 SELECT YOUR TEAM", "32px " + FONT_GAME, TEXT_COLOR);
		...
		oSelectTeamTextStroke = new createjs.Text("PLAYER 1 SELECT YOUR TEAM", "32px " + FONT_GAME, "#ff6000");
		...
		if (!_bMultiplayer)
		{
			oSelectTeamText.text = TEXT_SELECT_YOUR_TEAM;
			oSelectTeamTextStroke.text = TEXT_SELECT_YOUR_TEAM;
		}
    ...
    this._createFlag = function (i, iOffsetX, iOffsetY, iTimeWait, iTimeAnim, oSprite, oContainer) {
        ...
        _aPlayerTeamFlag[i].setScale(2);

        _aPlayerTeamFlag[i].showAnimation(iTimeWait, iTimeAnim);
    };

	this._createBigFlag = function (i, iOffsetX, iOffsetY, oSprite, oContainer) {
        _aPlayerTeamFlag[i] = new CGfxButton(iOffsetX, iOffsetY, oSprite, oContainer);
        _aPlayerTeamFlag[i].addEventListener(ON_MOUSE_UP, function(){ this._onButTeamChoose(i)}, this);
    };
    ...
    this._onButTeamChoose = function (iID) {
        if (_iActiveTeam !== iID) {
			...
			if (_bMultiplayer)
			{
				if (!_bPlayerOneSelected)
				{
					_iActiveTeam = iID;
					_bPlayerOneSelected = true;
					oSelectTeamText.text = "PLAYER 2 SELECT YOUR TEAM";
					oSelectTeamTextStroke.text = "PLAYER 2 SELECT YOUR TEAM";
					
				}else{
					_iActiveTeamTwo = iID;
					_bPlayerTwoSelected = true;
				}
			}
			else{
				_iActiveTeam = iID;
    ...
    this._onButContinueRelease = function () {
		if (_bMultiplayer)
		{
			if (_bPlayerOneSelected && _bPlayerTwoSelected)
			{
				this.unload();
				s_oMain.gotoGame(_iActiveTeam, _iActiveTeamTwo);
			}
		}
		else
		{
