function CPlayer(iXPos, iYPos, oSprite, iID, bUserPlayer, bGoalKeeper, iType, oParentContainer) {
    ...
    this._init = function (iXPos, iYPos, iID, bUserPlayer, bGoalKeeper, iType, oSprite) {
        ...
        var oData = {
            ...
            frames: {width: oSprite.width / 12, height: oSprite.height / 4, regX: (oSprite.width / 2) / 12, regY: oSprite.height / 4 + PLAYER_REGY_OFFSET}
        };
        var oSpriteSheet = new createjs.SpriteSheet(oData);
        _oPlayer = createSprite(oSpriteSheet, 0, (oSprite.width / 2) / 12, oSprite.height / 4 + PLAYER_REGY_OFFSET, oSprite.width / 12, oSprite.height / 4);
        _oPlayer.stop();
		this.onSelectHandler = this.onSelect.bind(this);
		this.onPressMoveHandler = this.onPressMove.bind(this);
    ...
    this._initListner = function () {
       _oPlayer.addEventListener("mousedown", this.onSelectHandler);
       _oPlayer.addEventListener("pressmove", this.onPressMoveHandler);
       _oPlayer.addEventListener("pressup", this.onPressUpHandler);
    };
	...
	this.turnListenerOff = function() {
		_oPlayer.removeEventListener("mousedown", this.onSelectHandler);
        _oPlayer.removeEventListener("pressmove", this.onPressMoveHandler);
        _oPlayer.removeEventListener("pressup", this.onPressUpHandler);
	};
	this.turnListenerOn = function() {
		_oPlayer.addEventListener("mousedown", this.onSelectHandler);
        _oPlayer.addEventListener("pressmove", this.onPressMoveHandler);
        _oPlayer.addEventListener("pressup", this.onPressUpHandler);
	};
    ...
    this.unload = function () {
        if (_bUserPlayer) {
            _oPlayer.off("mousedown", this.onSelectHandler);
            _oPlayer.off("pressmove", this.onPressMoveHandler);
            _oPlayer.off("pressup", this.onPressUpHandler);
        }
