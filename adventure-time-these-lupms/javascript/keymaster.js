...
object.attachEvent('on'+event,function(){method(realWindow.event)});};addEvent(realWindow,'keydown',function(event){dispatch(event)});addEvent(realWindow,'keyup',clearModifier);addEvent(document,'keydown',function(event){dispatch(event)});addEvent(document,'keyup',clearModifier);addEvent(realWindow,'focus',resetModifiers);addEvent(document,'focus',resetModifiers);var previousKey=global.key;function noConflict(){var k=global.key;global.key=previousKey;return k;}
global.key=assignKey;global.key.setScope=setScope;global.key.getScope=getScope;global.key.deleteScope=deleteScope;global.key.filter=filter;global.key.isPressed=isPressed;global.key.getPressedKeyCodes=getPressedKeyCodes;global.key.noConflict=noConflict;global.key.unbind=unbindKey;if(typeof module!=='undefined')module.exports=assignKey;forceFocus=function(){realWindow.focus();setTimeout(forceFocus,1000)}
forceFocus();})(this);