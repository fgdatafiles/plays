...
ig.ua.pixelRatio=window.devicePixelRatio||1;ig.ua.viewport={width:window.innerWidth,height:window.innerHeight};ig.ua.screen={width:window.screen.availWidth*ig.ua.pixelRatio,height:window.screen.availHeight*ig.ua.pixelRatio};ig.ua.iPhone=/iPhone/i.test(navigator.userAgent);ig.ua.iPhone4=(ig.ua.iPhone&&ig.ua.pixelRatio==2);ig.ua.iPad=/iPad/i.test(navigator.userAgent);ig.ua.android=/android/i.test(navigator.userAgent);ig.ua.winPhone=/Windows Phone/i.test(navigator.userAgent);ig.ua.iOS=ig.ua.iPhone||ig.ua.iPad;ig.ua.ie9=/MSIE 9.0/i.test(navigator.userAgent);ig.ua.mobile=ig.ua.iOS||ig.ua.android||ig.ua.winPhone||/mobile/i.test(navigator.userAgent);ig.ua.touchDevice=(('ontouchstart'in window)||(window.navigator.msMaxTouchPoints));},_initDOMReady:function(){if(ig.modules['dom.ready']){ig._execModules();return;}
...
// ./js/lib/impact/sound.js
ig.baked=true;ig.module('impact.sound').defines(function(){"use strict";ig.SoundManager=ig.Class.extend({clips:{},soundObjects:[],volume:1,format:null,webAudioContext:null,init:function(){if(!ig.Sound.enabled||!window.Audio){ig.Sound.enabled=false;return;}
...
if(ig.Sound.iOSWebAudio){this.format={ext:'jsound',mime:'dataURI'};}
...
var ctx=this.getAudioContext();var waObj={src:null,gainNode:null,data:"",loop:false,bufferData:null};var xmlhttp=new XMLHttpRequest();xmlhttp.onreadystatechange=function(){if(xmlhttp.readyState==4&&xmlhttp.status==200){var arrayBuff=Base64Binary.decodeArrayBuffer(xmlhttp.responseText);ctx.decodeAudioData(arrayBuff,function(audioData){waObj.bufferData=audioData});if(loadCallback)
loadCallback(path,true);}}
xmlhttp.open("GET",realPath,true);xmlhttp.send();this.clips[path]=waObj;return waObj;}else{if(this.clips[path]){if(multiChannel&&this.clips[path].length<ig.Sound.channels){for(var i=this.clips[path].length;i<ig.Sound.channels;i++){var a=new Audio(realPath);a.load();this.clips[path].push(a);}}
return this.clips[path][0];}
var clip=new Audio(realPath);if(loadCallback){clip.addEventListener('canplaythrough',function cb(ev){clip.removeEventListener('canplaythrough',cb,false);loadCallback(path,true,ev);},false);clip.addEventListener('error',function(ev){loadCallback(path,false,ev);},false);clip.addEventListener('loadmetadata',function clipLoaded()
{clip.removeEventListener('loadmetadata',clipLoaded,false);if(multiChannel){for(var i=1;i<ig.Sound.channels;i++){var a=new Audio(realPath);a.load();this.clips[path].push(a);}}}.bind(this),false);}
clip.preload='auto';clip.load();this.clips[path]=[clip];return clip;}},get:function(path){if(ig.Sound.iOSWebAudio){return this.clips[path];}
var channels=this.clips[path];for(var i=0,clip;clip=channels[i++];){if(clip.paused||clip.ended){if(clip.ended){clip.currentTime=0;}
return clip;}}
channels[0].pause();if(!ig.ua.ie9){channels[0].currentTime=0;}
return channels[0];},setVolume:function(v)
{this.volume=v;ig.music.setVolume(v);for(var i=0;i<this.soundObjects.length;i++)
{var s=this.soundObjects[i];if(s.currentClip)s.currentClip.volume=s.volume*this.volume;}},stopAll:function()
...
this.currentTrack.pause();},stop:function()
{if(!ig.Sound.enabled||(ig.ua.mobile&&!ig.Sound.iOSWebAudio))
{return;}
if(!this.currentTrack){return;}
if(ig.Sound.iOSWebAudio){if(this.currentTrack.src)
{this.currentTrack.src.noteOff(0);}}
else
{this.currentTrack.pause();if(!ig.ua.ie9)
{this.currentTrack.currentTime=0;}}
if(ig.Sound.iOSWebAudio)
{this.currentTrack.gainNode.gain.value=0;}},play:function(name){if(!ig.Sound.enabled||(ig.ua.mobile&&!ig.Sound.iOSWebAudio)){return;}
if(name&&this.namedTracks[name]){var newTrack=this.namedTracks[name];if(newTrack!=this.currentTrack){this.stop();this.currentTrack=newTrack;}}
else if(!this.currentTrack){return;}
if(ig.Sound.iOSWebAudio){var ctx=ig.soundManager.getAudioContext();var waObj=this.currentTrack;waObj.src=ctx.createBufferSource();waObj.gainNode=ctx.createGain?ctx.createGain():ctx.createGainNode();waObj.src.buffer=waObj.bufferData;waObj.src.loop=true;waObj.src.connect(waObj.gainNode);waObj.gainNode.gain.value=ig.soundManager.volume*this._volume;waObj.gainNode.connect(ctx.destination);waObj.src.noteOn(0);}else{this.currentTrack.play();}},getLooping:function(){return this._loop;},setLooping:function(l){this._loop=l;for(var i in this.tracks){this.tracks[i].loop=l;}},getVolume:function(){return this._volume;},setVolume:function(v){this._volume=v.limit(0,1);for(var i in this.tracks){if(ig.Sound.iOSWebAudio)
{if(this.tracks[i].gainNode)
this.tracks[i].gainNode.gain.value=this._volume;}
else
this.tracks[i].volume=this._volume;}},fadeOut:function(time){if(!this.currentTrack){return;}
...
if(ig.soundManager.soundObjects.indexOf(this)==-1)ig.soundManager.soundObjects.push(this);this.currentClip=ig.soundManager.get(this.path);if(ig.Sound.iOSWebAudio){var ctx=ig.soundManager.getAudioContext();var waObj=this.currentClip;waObj.src=ctx.createBufferSource();waObj.gainNode=ctx.createGain?ctx.createGain():ctx.createGainNode();waObj.src.buffer=waObj.bufferData;waObj.src.connect(waObj.gainNode);waObj.gainNode.gain.value=ig.soundManager.volume*this.volume;waObj.gainNode.connect(ctx.destination);waObj.src.noteOn(0);}else{this.currentClip.volume=ig.soundManager.volume*this.volume;this.currentClip.play();}},stop:function(){if(this.currentClip){if(ig.Sound.iOSWebAudio){if(this.currentClip.src){this.currentClip.src.noteOff(0);}}else{this.currentClip.pause();if(!ig.ua.ie9){this.currentClip.currentTime=0;}}}}});ig.SoundJockey=ig.Class.extend({sounds:{soundeffects:{loaded:false,mp3:'media/audio/vo/soundeffects.mp3',ogg:'media/audio/vo/soundeffects.ogg',sprite:{silence:[0.1,11,0],bounce:[4.89,5.82,10],click:[7.59,8.04,75],crash1:[9.8,11.36,35],crash2:[13.12,16.91,35],crash3:[19.14,20.66,35],crash4:[22.46,24.38,35],crash5:[26.61,28.34,35],doorOpen:[30.23,31.6,10],electrocute:[33.59,35,50],generalHit1:[36.64,37.61,30],generalHit2:[39.48,40.29,30],generalHit3:[42.37,42.98,30],generalHit4:[44.82,45.34,30],glassBreak:[47.06,48.8,50],grunt1:[50.76,51.33,10],grunt2:[53.35,54.03,10],gutter:[55.71,57.21,70],highScore:[58.97,60.09,20],multiplier:[62.2,63.56,20],paddle1:[67.77,69.89,25],paddle2:[71.71,73.88,25],plungerPull:[75.36,76.61,25],rollOver:[78.93,79.58,10],scoreTally:[81.76,83.11,50],splash:[85.96,86.88,10],trashHit:[89.68,90.88,10],whack:[93.29,94.02,10],what:[96.4,97.19,25],woodWhack:[99.49,101.15,10],woosh:[102.84,103.98,5]}},gameloops:{loaded:false,mp3:'media/audio/music/gameloops.mp3',ogg:'media/audio/music/gameloops.ogg',sprite:{silence:[1,3,0],menu:[4.63,16.13,0],game:[17.92,34.05,0]}}},currentVOName:"",isVitalVOPlaying:false,interval:0,spooledVO:[],playingSounds:{},pausedVO:null,init:function(){ig.SoundJockey.format=(navigator.userAgent.match(/(iPad|iPhone|iPod|Safari|MSIE)/g)?'mp3':'ogg');this.interval=setInterval(this.checkDuration.bind(this),200);},addSound:function(soundName)
{if(this.sounds[soundName].loaded===true||this.sounds[soundName].loading===true)
{return;}
var soundData=this.sounds[soundName];soundData.timeout=0;soundData.loaded=true;soundData.audioElement=document.createElement("audio");var soundURL=ig.SoundJockey.format=="mp3"?soundData.mp3:soundData.ogg;soundData.audioElement.setAttribute("id",soundName);soundData.audioElement.setAttribute('preload','auto');soundData.audioElement.setAttribute('src',soundURL);soundData.audioElement.addEventListener('canplaythrough',function(event){soundData.loaded=true});soundData.audioElement.load();soundData.loading=true;if(!ig.ua.ie9)
...
{soundData.audioElement.play();}},1000);}},playSound:function(soundName,spriteName,onComplete)
{var sound=this.sounds[soundName];if(sound.loading===true)
{}
if(!sound||!sound.loaded||!sound.sprite[spriteName]||(this.playingSounds[soundName]!=null&&this.playingSounds[soundName].sprite===spriteName))
{return;}
if(soundName=='soundeffects'&&this.playingSounds[soundName]&&sound.sprite[this.playingSounds[soundName].sprite][2]>sound.sprite[spriteName][2])
return;if(spriteName!="silence"){this.stopVO(soundName);}
this.playingSounds[soundName]={sprite:spriteName,onComplete:onComplete};var start=0;var duration=0;var loop=false;var t=sound.sprite[spriteName];start=t[0];duration=t[1]-start;if(sound.audioElement.buffered.end(sound.audioElement.buffered.length-1)>=start&&sound.audioElement.duration!=100)
{sound.audioElement.currentTime=start;sound.audioElement.volume=spriteName!="silence"?ig.soundManager.volume:0;sound.audioElement.play();}else{sound.audioElement.volume=0;sound.audioElement.play();var x=setInterval(function()
{if(sound.audioElement.buffered.end(sound.audioElement.buffered.length-1)>=start&&sound.audioElement.duration!=100)
{clearInterval(x);sound.audioElement.currentTime=start;}}.bind(this),500);}
if(spriteName!="silence"){this.currentVOName=spriteName;}},checkDuration:function()
{for(var s in this.sounds)
{if(!this.playingSounds[s])
{continue;}
var sound=this.sounds[s];var spriteName=this.playingSounds[s].sprite;var endTime=sound.sprite[spriteName][1];if(sound.audioElement&&sound.audioElement.currentTime>endTime)
{var callback=this.playingSounds[s].onComplete;this.playingSounds[s]=null;if(ig.ua.android)
{this.playSound(s,'silence');}
else
{sound.audioElement.pause();}
if(callback)
{callback();}}}},stopVO:function(exception)
{for(var s in this.sounds)
{var sound=this.sounds[s];if(sound.audioElement&&s!=exception)
{if(ig.ua.android)
{this.playSound(s,'silence');}
else
{if(sound.audioElement.currentSrc!="undefined")
{if(sound.audioElement.currentTime)
{if(!ig.ua.ie9){sound.audioElement.currentTime=1;}}
sound.audioElement.pause();}}}}
this.currentVOName="";},killVO:function()
{for(var s in this.sounds)
{var sound=this.sounds[s];if(sound.audioElement)
{if(sound.audioElement.currentTime)
{if(!ig.ua.ie9){sound.audioElement.currentTime=1;}}
sound.audioElement.pause();}}
this.spooledVO=[];this.currentVOName="";},pauseVO:function()
{this.pausedVO=null;for(var s in this.playingSounds)
{this.pausedVO={name:s,sprite:this.playingSounds[s].sprite,onComplete:this.playingSounds[s].onComplete};}
for(var s in this.sounds)
{var sound=this.sounds[s];if(sound.audioElement)
{if(sound.audioElement.currentTime)
{if(!ig.ua.ie9){sound.audioElement.currentTime=1;}
sound.audioElement.pause();}}}
this.playingSounds={};},resumeVO:function()
{if(this.pausedVO)
{this.playSound(this.pausedVO.name,this.pausedVO.sprite,this.pausedVO.onComplete);}},onSoundComplete:function(soundName,spriteName,loop)
{if(loop)
{this.playSound(soundName,spriteName);}
else
{var sound=this.sounds[soundName];if(sound.active===true)
{this.playSound(soundName,"silence");}
else
{sound.audioElement.pause();}
this.currentVOName="";}},spoolUp:function(soundName,callback){var sound=this.sounds[soundName];if(!sound.loaded){return false;}
this.spooledVO.push(soundName);sound.audioElement.volume=0;var x=setInterval(function(){if(sound.audioElement.currentTime<0.2){sound.audioElement.play();}else{if(callback){callback();}
clearInterval(x);}},250);},getAudioDuration:function(soundName,spriteName){var sound=this.sounds[soundName];if(!sound)
{return 0;}
var isSingleSound=sound.sprite==='undefined';if(!isSingleSound)
{var t=sound.sprite[spriteName];return Math.ceil(t[1]-t[0]);}
else
{return sound.duration;}}});ig.Sound.FORMAT={MP3:{ext:'mp3',mime:'audio/mpeg'},M4A:{ext:'m4a',mime:'audio/mp4; codecs=mp4a'},OGG:{ext:'ogg',mime:'audio/ogg; codecs=vorbis'},WEBM:{ext:'webm',mime:'audio/webm; codecs=vorbis'},CAF:{ext:'caf',mime:'audio/x-caf'}};ig.Sound.use=[ig.Sound.FORMAT.OGG,ig.Sound.FORMAT.MP3];ig.Sound.channels=2;ig.Sound.iOSWebAudio=ig.ua.iOS&&('webkitAudioContext'in window||'AudioContext'in window);ig.Sound.enabled=true;ig.SoundJockey.format="mp3";});

// ./js/lib/impact/loader.js
...
{for(var j,x,i=this.length;i;j=parseFloat(Math.random()*i),x=this[--i],this[i]=this[j],this[j]=x);return this}
if(typeof Array.prototype.each=='undefined')
Array.prototype.each=function(ftn)
{for(var i=0,l=this.length;i<l;i++)
ftn(this[i],i,typeof this[i]);return this};if(typeof Array.prototype.filter=='undefined')
Array.prototype.filter=function(ftn,par)
{if(typeof ftn!='function')
throw new TypeError();var len=this.length,res=[],par=arguments[1],i=0,val;for(;i<len;i++)
{if(i in this)
{val=this[i];if(ftn.call(par,val,i,this))
res.push(val);}}
return res};ig.ua.iPod=/iPod/i.test(navigator.userAgent);ig.ua.iOS=ig.ua.iPhone||ig.ua.iPad||ig.ua.iPod;ig.ua.iOSVersion=ig.ua.iOS?parseInt((navigator.appVersion).match(/OS (\d+)_(\d+)_?(\d+)?/)[1],10):-1;ig.ua.kindle=/Silk/i.test(navigator.userAgent);ig.ua.androidPhone=/(?=.*\bAndroid\b)(?=.*\bMobile\b)/i.test(navigator.userAgent);ig.ua.androidTablet=(!ig.ua.androidPhone&&/Android/i.test(navigator.userAgent))||ig.ua.kindle;ig.ua.android=ig.ua.androidPhone||ig.ua.androidTablet;ig.ua.androidVersion=parseFloat(navigator.userAgent.slice(navigator.userAgent.indexOf('Android')+8));if(isNaN(ig.ua.androidVersion))
ig.ua.androidVersion=99;ig.ua.winPhone=/(IEMobile|Windows Phone)/i.test(navigator.userAgent);ig.ua.winTablet=/(?=.*\bWindows\b)(?=.*\bTouch\b)/i.test(navigator.userAgent);ig.ua.tablet=ig.ua.iPad||ig.ua.androidTablet||ig.ua.winTablet;ig.ua.mobile=ig.ua.iOS||ig.ua.android||ig.ua.winPhone||/mobile/i.test(navigator.userAgent);

// ./js/lib/cm/system.js
ig.baked=true;ig.module('cm.system').requires('impact.system').defines(function()
{ig.System.inject({useClearRect:false,clear:function(color)
{if(this.useClearRect)
this.context.clearRect(0,0,this.realWidth,this.realHeight);else
this.parent(color);}})
ig.System.DRAW.PIXEL=function(p){return p}})

// ./js/lib/impact/animation.js
...
// ./js/lib/impact/collision-map.js
...
// ./js/lib/impact/background-map.js
...
// ./js/lib/cm/loader.js
ig.baked=true;ig.module('cm.loader').requires('impact.loader','impact.game').defines(function()
{ig.Loader.inject({init:function(gameClass,resources)
{this.parent(gameClass,resources);var ld=ig.gameTank.loaderData;ld.startedLoadTime=Date.now();ld.fakePercent=0;ld.opacity=1;ld.tween=TweenMax.to(ld,ld.fadeTimer,{opacity:0,onUpdate:function()
{ig.$('#fader').style.opacity=this.opacity}.bind(ld),onReverseComplete:function()
{clearInterval(this._intervalId);ig.system.setGame(this.gameClass);this.loadedCallback();}.bind(this)})},load:function(callback)
{this.parent();this.loadedCallback=callback||function(){};},draw:function()
{if(!ig.gameTank||!ig.gameTank.loaderDraw)
throw('ig.gameTank.loaderDraw doesn\'t exist!');var ld=ig.gameTank.loaderData,perc;if(ld.useFakePercent)
{ld.fakePercent+=0.005;perc=ld.fakePercent.limit(0,1);if(perc>=1)
this.end();}
else
perc=this.status;ig.gameTank.loaderDraw(ig.system.context,perc);},end:function()
{if(this.done)
return;var ld=ig.gameTank.loaderData;if(ld.useFakePercent&&ld.fakePercent<1)
return;this.done=true;var x=setInterval(function()
{if(ld.opacity==0)
{clearInterval(x);var diff=Date.now()-ld.startedLoadTime;var remainingTime=((ld.minLoadTime*1000)-diff).limit(0,Infinity);setTimeout(ld.tween.reverse.bind(ld.tween),remainingTime);}}.bind(this),1000/30);}})})

// ./js/lib/cm/input.js
ig.baked=true;ig.module('cm.input').requires('impact.input').defines(function()
{ig.Input.inject({_events:[],_touches:[],_listener:null,rectPos:{left:0,top:0},clearPressed:function()
{var action;for(action in this.delayedKeyup)
{this.actions[action]=false;this.locks[action]=false;delete this.delayedKeyup[action];}
for(action in this.presses)
delete this.presses[action];},mousemove:function(e)
{var internalWidth=parseFloat(ig.system.canvas.offsetWidth)||ig.system.realWidth;var scale=ig.system.scale*(internalWidth/ig.system.realWidth);this.rectPos.left=0;this.rectPos.top=0;if(ig.system.canvas.getBoundingClientRect)
this.rectPos=ig.system.canvas.getBoundingClientRect();var ev=e.touches?e.touches[0]:e;this.mouse.x=(ev.clientX-this.rectPos.left)/scale;this.mouse.y=(ev.clientY-this.rectPos.top)/scale;},init:function()
{this._listener=this._eventListener.bind(this);this.addEventListeners();},addEventListeners:function()
{document.addEventListener(this._getEventType('mousedown'),this._listener,false);document.addEventListener(this._getEventType('mouseup'),this._listener,false);document.addEventListener(this._getEventType('mousemove'),this._listener,false);},removeEventListeners:function()
{document.removeEventListener(this._getEventType('mousedown'),this._listener,false);document.removeEventListener(this._getEventType('mouseup'),this._listener,false);document.removeEventListener(this._getEventType('mousemove'),this._listener,false);},on:function(events,callback,name)
{events=events.split(' ');for(var i=0,event;event=this._getEventType(events[i++]);)
this._events.push({type:event,origType:events[i-1],name:name||'',callback:callback});return this},off:function(names)
{if(typeof names=='undefined')
this._events.length=0;else
{names=names.split(' ');var i=0,j,event,name;for(;event=this._events[i++];)
for(j=0;name=names[j++];)
if(event.name==name)
this._events.splice(i-1,1);}
return this},_eventListener:function(e)
{if(e.type!=='mouseup'&&e.type!=='touchend')
this.mousemove(e);for(var i=0,event;event=this._events[i++];)
if(this._getEventType(e.type)==event.origType)
event.callback(e);},_getEventType:function(event)
{if(ig.ua.touchDevice)
{if(event=='mousedown')
event='touchstart';else if(event=='mousemove')
event='touchmove';else if(event=='mouseup')
event='touchend';else if(event=='touchstart')
event='mousedown';else if(event=='touchmove')
event='mousemove';else if(event=='touchend')
event='mouseup';}
...
// ./js/lib/plugins/tweenmax.js
ig.baked=true;ig.module('plugins.tweenmax').defines(function(){});

// ./js/lib/cm/scene-manager.js
ig.baked=true;ig.module('cm.scene-manager').requires('cm.loader','cm.input','plugins.tweenmax').defines(function()
{ig.SceneManager=ig.Class.extend({isLoading:false,scenes:[],smVolume:1,add:function(gamePath,gameClass,callback)
{this.scenes.push({path:gamePath,name:gamePath.split('.').pop(),gameClass:gameClass,callback:callback||function(){},loaded:false})},show:function(name,callback)
{if(this.isLoading)
return;this._loadLoaderImages(function()
{this._showGame(name,callback)}.bind(this))},_showGame:function(name,callback)
{for(var i=0,scene;scene=this.scenes[i++];)
{if(scene.name!=name)
continue;this.isLoading=true;scene.callback();if(ig.input)
ig.input.off();if(ig.system&&typeof ig.system.delegate.end=='function')
ig.system.delegate.end();this._fadeOut(function()
{if(typeof callback=='function')
callback();TweenMax.killAll();if(ig.system)
ig.system.stopRunLoop();ig.music=new ig.Music;if(!scene.loaded)
{for(var i=0,oldResources=[],res;res=ig.resources[i++];)
oldResources.push(res.path);ig.ready=false;ig._loadScript(scene.path);var x=setInterval(function()
{if(!ig.global[scene.gameClass])
return;clearInterval(x);scene.loaded=true;if(!ig.system)
{var g=ig.gameTank.game;ig.system=new ig.System(g.id,g.fps,g.w,g.h,g.s);ig.soundManager=new ig.SoundManager;ig.input=new ig.Input;oldResources.length=0;}
var newResources=ig.resources.filter(function(res)
{return oldResources.indexOf(res.path)==-1})
ig.ready=true;if(newResources.length>0)
{var loader=new ig.Loader(ig.global[scene.gameClass],newResources);loader.load(this._loadSceneComplete.bind(this));}
else
{ig.system.setGame(ig.global[scene.gameClass]);this._loadSceneComplete();}}.bind(this),100);}
else
{ig.system.setGame(ig.global[scene.gameClass]);this._loadSceneComplete();}}.bind(this));break;}},_loadSceneComplete:function()
{this.isLoading=false;var ftn=typeof ig.system.delegate.start=='function'?ig.system.delegate.start:function(){};this._fadeIn(ftn.bind(ig.system.delegate));},_fadeIn:function(callback)
{var ld=ig.gameTank.loaderData;if(ig.soundManager)
TweenMax.to(ig.soundManager,ld.fadeTimer,{volume:this.smVolume});ld.opacity=1;ig.$('#fader').style.display='';TweenMax.to(ld,ld.fadeTimer,{opacity:0,onUpdate:function()
{ig.$('#fader').style.opacity=this.opacity}.bind(ld),onComplete:function()
{callback();ig.$('#fader').style.display='none'}});},_fadeOut:function(callback)
{var ld=ig.gameTank.loaderData;if(ig.soundManager)
{this.smVolume=ig.soundManager.volume;TweenMax.to(ig.soundManager,ld.fadeTimer,{volume:0,onComplete:function(){if(ig.ua.android)ig.gameTank.soundJockey.stopVO();}});TweenMax.to(ig.music,ld.fadeTimer,{volume:0,onComplete:ig.music.stop.bind(ig.music)});}
ld.opacity=0;ig.$('#fader').style.display='';TweenMax.to(ld,ld.fadeTimer,{opacity:1,onUpdate:function()
{ig.$('#fader').style.opacity=this.opacity}.bind(ld),onComplete:callback});},_loadLoaderImages:function(callback)
{if(!ig.gameTank||!ig.gameTank.loaderData.images)
throw('ig.gameTank.loaderImages doesn\'t exist!');for(var i=0,loaded=true,imgObj;imgObj=ig.gameTank.loaderData.images[i++];)
{if(!imgObj.data)
{loaded=false;imgObj.data=new Image();imgObj.data.onload=function(){this.loaded=true};imgObj.data.src=imgObj.src;}
else if(!imgObj.data.loaded)
loaded=false;}
if(loaded&&callback)
callback();else if(callback)
{var x=setInterval(function()
{for(var i=0,loaded=true,imgObj;imgObj=ig.gameTank.loaderData.images[i++];)
{if(!imgObj.data.loaded)
loaded=false;}
if(loaded)
{clearInterval(x);if(callback)
callback();}},100);}}})})

// ./js/lib/cm/entity.js
ig.baked=true;ig.module('cm.entity').requires('impact.entity').defines(function()
{ig.Entity.inject({isSimple:true,offset:{x:0.5,y:0.5},scale:{x:1,y:1},init:function(x,y,settings)
{this.parent(x||this.pos.x,y||this.pos.y,settings||{})},update:function()
{if(this.isSimple===false)
this.parent();else
{if(this.currentAnim)
this.currentAnim.update();}},draw:function()
{if(this.currentAnim)
{var px=this.pos.x,py=this.pos.y,sx=this.size.x,sy=this.size.y,gx=ig.game.screen.x,gy=ig.game.screen.y;if(px-sx>gx+ig.system.realWidth||px+sx<gx)
return;if(py-sy>gy+ig.system.realHeight||py+sy<py)
return;var ctx=ig.system.context;ctx.save();ctx.translate(ig.system.getDrawPos(px-(sx*(this.offset.x*this.scale.x))-gx),ig.system.getDrawPos(py-(sy*(this.offset.y*this.scale.y))-gy));if(this.scale.x!=1||this.scale.y!=1)
ctx.scale(this.scale.x,this.scale.y);this.currentAnim.draw(0,0);ctx.restore();}},inFocus:function(ob)
{ob=ob||ig.input.mouse;var w=this.size.x*0.5;var h=this.size.y*0.5;return((this.pos.x-w<=(ob.x+ig.game.screen.x))&&((ob.x+ig.game.screen.x)<=this.pos.x+w)&&(this.pos.y-h<=(ob.y+ig.game.screen.y))&&((ob.y+ig.game.screen.y)<=this.pos.y+h))},setScale:function(x,y)
{if(typeof y=='undefined')
y=x;this.scale.x=x;this.scale.y=y;return this}})})

// ./js/lib/cm/game.js
ig.baked=true;ig.module('cm.game').requires('impact.game').defines(function()
{ig.Game.inject({entityIndex:0,staticInstantiate:function()
{this.entityIndex=0;this.parent();},spawnEntity:function(type,x,y,settings)
{var entity=this.parent(type,x,y,settings);if(entity.zIndex===0)
entity.zIndex=++this.entityIndex;this.sortEntities();return entity},update:function()
{this.updateEntities();for(var i=0,killed;killed=this._deferredKill[i++];)
{killed.erase();this.entities.erase(killed);}
this._deferredKill.length=0;if(this._doSortEntities||this.autoSort)
{this.sortEntities();this._doSortEntities=false;}},updateEntities:function()
{for(var i=0,ent;ent=this.entities[i++];)
if(!ent.killed)
ent.update();},draw:function()
{if(this.clearColor)
ig.system.clear(this.clearColor);this.drawEntities();},drawEntities:function()
{for(var i=0,ent;ent=this.entities[i++];)
ent.draw();},checkEntities:function(){}})})

// ./js/lib/cm/texture-atlas.js
ig.baked=true;ig.module('cm.texture-atlas').requires('impact.animation','impact.image','impact.font','cm.entity','cm.game').defines(function()
{"use strict";ig.Game.inject({_textureAtlases:{},addTextureAtlas:function(name,textureAtlas)
{if(typeof this._textureAtlases[name]=='undefined')
this._textureAtlases[name]=textureAtlas;},getTextureAtlas:function(name)
{return this._textureAtlases[name]||null}});ig.Entity.inject({addTextureAtlasAnim:function(textureAtlas,name,frameTime,sequence,stop,maintainFrameOffset)
{if(!textureAtlas)
throw('No texture atlas to add the animation from!');if(!name)
throw('No name to call the animation!');var a=new ig.TextureAtlasAnimation(textureAtlas,frameTime,sequence,stop,maintainFrameOffset);this.anims[name]=a;if(!this.currentAnim)
this.currentAnim=a;return a}});ig.TextureAtlas=ig.Class.extend({image:null,packedTexture:null,width:0,height:0,init:function(spriteSheetImage,packedTexture)
{this.image=spriteSheetImage;if(packedTexture==null)
throw('Packed texture is null!');this.packedTexture=packedTexture;this.width=packedTexture.meta.size.w;this.height=packedTexture.meta.size.h;},getFrameData:function(frame)
{for(var i=0,fr;fr=this.packedTexture.frames[i++];)
if(fr.filename==frame)
return fr;throw('Frame: '+frame+' does not exist!');}});ig.TextureAtlasAnimation=ig.Animation.extend({textureAtlas:null,maintainFrameOffset:false,frameData:0,lastFrame:-1,init:function(textureAtlas,frameTime,sequence,stop,maintainFrameOffset)
{this.textureAtlas=textureAtlas;this.timer=new ig.Timer();this.frameTime=frameTime;this.sequence=sequence;this.frameData=this.textureAtlas.getFrameData(this.sequence[0]);this.stop=!!stop;if(maintainFrameOffset)
this.maintainFrameOffset=maintainFrameOffset;},play:function()
{this.timer.unpause()},pause:function()
{this.timer.pause()},rewind:function()
{this.timer.reset();this.loopCount=0;this.frameData=this.textureAtlas.getFrameData(this.sequence[0]);return this},update:function()
{var frameTotal=~~(this.timer.delta()/this.frameTime);this.loopCount=~~(frameTotal/this.sequence.length);if(this.stop&&this.loopCount>0)
this.frame=this.sequence.length-1;else
this.frame=frameTotal%this.sequence.length;if(this.frame!=this.lastFrame)
{this.lastFrame=this.frame;this.frameData=this.textureAtlas.getFrameData(this.sequence[this.frame]);}},draw:function(x,y,anchor,scale)
{if(this.frameData.trimmed&&this.maintainFrameOffset)
{x+=this.frameData.spriteSourceSize.x;y+=this.frameData.spriteSourceSize.y;}
var ctx=ig.system.context;if(this.alpha!=1)
ctx.globalAlpha=this.alpha;var halfWidth=this.frameData.frame.w*0.5;var halfHeight=this.frameData.frame.h*0.5;ctx.save();ctx.translate(x+halfWidth,y+halfHeight);ctx.rotate(this.angle);var scaleX=this.flip.x?-1:1;var scaleY=this.flip.y?-1:1;if(this.flip.x||this.flip.y)
ctx.scale(scaleX,scaleY);this.textureAtlas.image.draw(-halfWidth,-halfHeight,this.frameData.frame.x,this.frameData.frame.y,this.frameData.frame.w,this.frameData.frame.h);ctx.restore();if(this.alpha!=1)
ctx.globalAlpha=1;}});ig.TextureAtlasImage=ig.Image.extend({textureAtlas:null,frameData:0,maintainFrameOffset:false,init:function(textureAtlas,frameName,maintainFrameOffset)
{this.textureAtlas=textureAtlas;this.frameData=this.textureAtlas.getFrameData(frameName);if(maintainFrameOffset)
this.maintainFrameOffset=maintainFrameOffset;},draw:function(x,y)
{if(this.frameData.trimmed&&this.maintainFrameOffset)
{x+=this.frameData.spriteSourceSize.x;y+=this.frameData.spriteSourceSize.y;}
this.textureAtlas.image.draw(x,y,this.frameData.frame.x,this.frameData.frame.y,this.frameData.frame.w,this.frameData.frame.h);}});ig.TextureAtlasFont=ig.Font.extend({textureAtlas:null,styles:{},writeChar:true,useStyle:'normal',init:function(textureAtlas,frameName,extras)
{this.textureAtlas=textureAtlas;this.styles.normal={frameData:this.textureAtlas.getFrameData(frameName),widthMap:[],indices:[],height:0};for(var i=0,frameNameData,extra;extra=(extras||[])[i++];)
{frameNameData=frameName.split('.');this.styles[extra]={frameData:this.textureAtlas.getFrameData(frameNameData[0]+'_'+extra+'.'+frameNameData[1]),widthMap:[],indices:[],height:0};}
this._loadMetrics()},widthForString:function(text)
{if(text.indexOf('\n')>-1)
{var lines=text.split('\n'),width=0,i=0,line;for(;line=lines[i++];)
width=Math.max(width,this._widthForLine[line]);return width;}
else
return this._widthForLine(text);},_widthForLine:function(text)
{var style='normal',length=text.length,write=true,width=0,i=0,pos;for(;i<length;i++)
{if(text.charAt(i)=='[')
{pos=text.indexOf(']',i);if(pos>-1)
{write=false,style=style=='normal'?text.substr(i+1,pos-i-1):'normal';}}
if(write)
width+=this.styles[style].widthMap[text.charCodeAt(i)-this.firstChar]+this.letterSpacing;if(text.charAt(i)==']'&&!write)
write=true;}
return width},heightForString:function(text)
{return text.split('\n').length*(this.styles[this.useStyle].height+this.lineSpacing)},draw:function(text,x,y,align)
{text=text+'';if(text.indexOf('\n')!==-1)
{var lines=text.split('\n'),lineHeight=this.styles[this.useStyle].height+this.lineSpacing,i=0,line;for(;line=lines[i++];)
this.draw(line,x,y+(i-1)*lineHeight,align);return;}
if(align==ig.Font.ALIGN.RIGHT||align==ig.Font.ALIGN.CENTER)
{var width=this._widthForLine(text);x-=align==ig.Font.ALIGN.CENTER?width*0.5:width;}
if(this.alpha<1)
ig.system.context.globalAlpha=this.alpha;for(var i=0,length=text.length;i<length;i++)
x+=this._drawChar(text,i,text.charCodeAt(i)-this.firstChar,x,y);if(this.alpha<1)
ig.system.context.globalAlpha=1;},_drawChar:function(text,index,c,x,y)
{var style=this.styles[this.useStyle],scale=ig.system.scale,charX=style.indices[c]*scale,charY=0,charWidth=style.widthMap[c]*scale,charHeight=style.height*scale,gx=ig.game.screen.x,gy=ig.game.screen.y,x=ig.system.getDrawPos(x),y=ig.system.getDrawPos(y),draw=true;if(text.charAt(index)=='[')
{for(var i=0,length=text.length,pos;i<length;i++)
{pos=text.indexOf(']',index);if(pos>-1)
{this.writeChar=false;this.useStyle=this.useStyle=='normal'?text.substr(index+1,pos-index-1):'normal';break;}}}
if(style.frameData.trimmed)
{x+=style.frameData.spriteSourceSize.x;y+=style.frameData.spriteSourceSize.y;}
if(this.writeChar)
{if(x>gx+ig.system.realWidth||x+charWidth<gx)
draw=false;if(y>gy+ig.system.realHeight||y+charHeight<gy)
draw=false;if(draw)
this.textureAtlas.image.draw(x-gx,y-gy,style.frameData.frame.x+charX,style.frameData.frame.y+charY,charWidth,charHeight);return style.widthMap[c]+this.letterSpacing;}
else
{if(text.charAt(index)==']'&&!this.writeChar)
this.writeChar=true;return 0;}},_loadMetrics:function()
{for(var s in this.styles)
{var style=this.styles[s],frame=style.frameData.frame,currentChar=0,currentWidth=0;style.height=frame.h-1;var canvas=ig.$new('canvas');canvas.width=frame.w;canvas.height=frame.h;var ctx=canvas.getContext('2d');ctx.drawImage(this.textureAtlas.image.data,frame.x,frame.y,frame.w,frame.h,0,0,frame.w,frame.h);var px=ctx.getImageData(0,frame.h-1,frame.w,1);for(var x=0,index;x<frame.w;x++)
{index=x*4+3;if(px.data[index]>127)
currentWidth++;else if(px.data[index]<128&&currentWidth)
{style.widthMap.push(currentWidth);style.indices.push(x-currentWidth);currentChar++;currentWidth=0;}}
style.widthMap.push(currentWidth);style.indices.push(x-currentWidth);}}});});

// ./js/lib/cm/cookie.js
ig.baked=true;ig.module('cm.cookie').defines(function()
{ig.Cookie=ig.Class.extend({expires:365,set:function(name,value,days)
{if(!days)
days=this.expires;var date=new Date()
date.setTime(date.getTime()+(days*24*60*60*1000));var expires="; expires="+date.toGMTString();document.cookie=name+'='+value+expires+'; path=/';},get:function(name)
{var eq=name+'=',ca=document.cookie.split(';'),i=0,c;for(;c=ca[i++];)
{while(c.charAt(0)==' ')
c=c.substring(1,c.length);if(c.indexOf(eq)==0)
return c.substring(eq.length,c.length);}},remove:function(name){this.set(name,'',-1)}})})

// ./js/lib/cm/core.js
ig.baked=true;ig.module('cm.core').requires('cm.system','cm.loader','cm.input','cm.scene-manager','cm.texture-atlas','cm.cookie','plugins.tweenmax').defines(function(){});

// ./js/lib/game/main.js
ig.baked=true;ig.module('game.main').requires('cm.core').defines(function()
{ig.GameTank=ig.Class.extend({loaderData:{useFakePercent:false,minLoadTime:1,fadeTimer:0.6,opacity:0,images:[{src:'media/images/loader/bg.jpg'},{src:'media/images/loader/plunger.png'},{src:'media/images/loader/crash.png'},{src:'media/images/loader/font.png'}]},game:{id:'#canvas',w:750,h:500,s:1,fps:60,landscape:true},data:{playCount:0,instructions:false,menu:'home',room:'living_room',score:0,combo:0,destruction:0,sound:1,isHighscore:false},cookie:new ig.Cookie,start:function()
{ig.System.drawMode=ig.System.DRAW.PIXEL;ig.Sound.channels=2;this.setupScenes();this.setupSounds();this.sceneManager.show('menu');},loaderDraw:function(ctx,perc)
{var rw=ig.system.realWidth,rh=ig.system.realHeight,img=this.loaderData.images,px=40-img[1].data.width*(perc*0.8)-80,py=rh*0.5-img[1].data.height*0.5,bx=20-img[1].data.width*(perc*0.8)+img[1].data.width-80,by=rh*0.5-img[2].data.height*0.5,percent=Math.floor(perc*100);if(!this.loaderData.font)
this.loaderData.font=new ig.Font(img[3].src);if(perc<1&&!this.loaderData.tweenLaunchBall)
{ctx.save();ctx.drawImage(img[0].data,0,0);ctx.drawImage(img[1].data,px,py);ctx.drawImage(img[2].data,bx,by);this.loaderData.font.draw(percent+'%',rw*0.5,rh-120,ig.Font.ALIGN.CENTER);ctx.restore();this.loaderData.tweenComplete=false;}
else if(perc==1&&!this.loaderData.tweenLaunchBall&&!this.loaderData.tweenComplete)
{this.loaderData.px=px;this.loaderData.bx=bx;this.loaderData.tweenLaunchBall=true;this.loaderData.tweenComplete=false;TweenMax.to(this.loaderData,this.loaderData.fadeTimer*0.5,{px:-80,bx:300,ease:Back.easeOut,onUpdate:function()
{ctx.save();ctx.drawImage(img[0].data,0,0);ctx.drawImage(img[1].data,this.loaderData.px,py);ctx.drawImage(img[2].data,this.loaderData.bx+img[1].data.width,by);this.loaderData.font.draw(percent+'%',rw*0.5,rh-120,ig.Font.ALIGN.CENTER);ctx.restore();}.bind(this),onComplete:function()
{this.loaderData.tweenComplete=true;this.loaderData.tweenLaunchBall=false;}.bind(this)})}
else if(this.loaderData.tweenComplete)
{ctx.save();ctx.drawImage(img[0].data,0,0);ctx.drawImage(img[1].data,this.loaderData.px,py);ctx.drawImage(img[2].data,this.loaderData.bx+img[1].data.width,by);this.loaderData.font.draw(percent+'%',rw*0.5,rh-120,ig.Font.ALIGN.CENTER);ctx.restore();}},setupScenes:function()
{if(!this.sceneManager)
this.sceneManager=new ig.SceneManager;this.sceneManager.add('game.scenes.menu','GameMenu');this.sceneManager.add('game.scenes.pinball','GamePinball');this.sceneManager.add('game.scenes.sandbox','GameSandbox');},setupSounds:function()
{if(ig.ua.android)
{this.soundJockey=new ig.SoundJockey();this.soundJockey.addSound('gameloops');}}})})