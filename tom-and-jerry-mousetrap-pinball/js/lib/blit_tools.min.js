...
break;case "static_fbx":my_loader=new THREE.FBXLoader(manager);my_loader.load(mdl_obj.src,function(json){trace("Processing Static FBX");var my_data=this.data;for(var i=0;i<json.children.length;i++){var group=json.children[i];if(group.name){group.traverse(function(node){if(node instanceof THREE.Mesh&&node.material.map){node.material.map.wrapS=THREE.RepeatWrapping;node.material.map.wrapT=THREE.RepeatWrapping;node.material.map.encoding=THREE.LinearEncoding;if(my_data.pixelated){node.material.map.minFilter=
THREE.NearestFilter;node.material.map.magFilter=THREE.NearestFilter}else if(platform.isMobile){node.material.map.minFilter=THREE.NearestMipMapNearestFilter;node.material.map.magFilter=THREE.NearestFilter}else{node.material.map.minFilter=THREE.LinearMipMapNearestFilter;node.material.map.magFilter=THREE.LinearFilter}if(my_data.flat_shading)node.material=new THREE.MeshBasicMaterial({map:node.material.map,blending:THREE.NoBlending,fog:false,flatShading:true,side:THREE.FrontSide});node.material.needsUpdate=
true}});lib[group.name]=group}}manager.items_to_process--;manager.doCheckLoading()}.bind({data:mdl_obj}));break;case "static":my_loader=new THREE.ObjectLoader(manager);my_loader.load(mdl_obj.src,function(json){var my_data=this.data;for(var i=0;i<json.children.length;i++){var group=json.children[i];if(group.name)if(group.name.includes(".DAE")||group.name.includes("ROOT"))for(var ii=0;ii<group.children.length;ii++){var child=group.children[ii];lib[child.name]=child}else{group.traverse(function(node){if(node instanceof
THREE.Mesh&&node.material.map){node.material.map.wrapS=THREE.RepeatWrapping;node.material.map.wrapT=THREE.RepeatWrapping;if(true){node.material.map.minFilter=THREE.NearestFilter;node.material.map.magFilter=THREE.NearestFilter}else if(platform.isMobile){node.material.map.minFilter=THREE.NearestMipMapNearestFilter;node.material.map.magFilter=THREE.NearestFilter}else{node.material.map.minFilter=THREE.LinearMipMapNearestFilter;node.material.map.magFilter=THREE.LinearFilter}}});lib[group.name]=group}}manager.items_to_process--;
manager.doCheckLoading()}.bind({name:mdl_obj.name}));break;case "animated_json":if(THREE.REVISION<100){my_loader=new THREE.JSONLoader(manager);my_loader.load(mdl_obj.src,function(geometry,materials){if(my_data.pixelated){materials[0].map.minFilter=THREE.NearestFilter;materials[0].map.magFilter=THREE.NearestFilter}else if(platform.isMobile){materials[0].map.minFilter=THREE.NearestMipMapNearestFilter;materials[0].map.magFilter=THREE.NearestFilter}else{materials[0].map.minFilter=THREE.NearestMipMapNearestFilter;
materials[0].map.minFilter=THREE.LinearFilter}materials[0].map.wrapS=THREE.ClampToEdgeWrapping;materials[0].map.wrapT=THREE.ClampToEdgeWrapping;var mat=new THREE.MeshBasicMaterial({map:materials[0].map});mat.skinning=true;lib[this.name]=new THREE.SkinnedMesh(geometry,mat);manager.items_to_process--;manager.doCheckLoading()}.bind({name:mdl_obj.name}))}else{my_loader=new THREE.LegacyJSONLoader(manager);my_loader.load(mdl_obj.src,function(geometry,materials){if(my_data.pixelated){materials[0].map.minFilter=
THREE.NearestFilter;materials[0].map.magFilter=THREE.NearestFilter}else if(platform.isMobile){materials[0].map.minFilter=THREE.NearestMipMapNearestFilter;materials[0].map.magFilter=THREE.NearestFilter}else{materials[0].map.minFilter=THREE.NearestMipMapNearestFilter;materials[0].map.minFilter=THREE.LinearFilter}materials[0].map.wrapS=THREE.ClampToEdgeWrapping;materials[0].map.wrapT=THREE.ClampToEdgeWrapping;var mat=new THREE.MeshBasicMaterial({map:materials[0].map});mat.skinning=true;var buffer_geom=
(new THREE.BufferGeometry).fromGeometry(geometry);lib[this.name]=new THREE.SkinnedMesh(buffer_geom,mat);manager.items_to_process--;manager.doCheckLoading()}.bind({name:mdl_obj.name}))}break;case "font":my_loader=new THREE.FontLoader(manager);my_loader.load(mdl_obj.src,function(font){lib[this.name]=font;manager.items_to_process--;manager.doCheckLoading()}.bind({name:mdl_obj.name}));break;case "sprite":my_loader=new THREE.TextureLoader(manager);my_loader.load(mdl_obj.src,function(map){var mat=new THREE.SpriteMaterial({map:map,
color:16777215,transparent:true});var sprite=new THREE.Sprite(mat);lib[this.name]=sprite;manager.items_to_process--;manager.doCheckLoading()}.bind({name:mdl_obj.name}));break;case "texture":my_loader=new THREE.TextureLoader(manager);my_loader.load(mdl_obj.src,function(map){var my_data=this.data;if(my_data.wrapping){map.wrapS=THREE.RepeatWrapping;map.wrapT=THREE.RepeatWrapping}lib[my_data.name]=map;manager.items_to_process--;manager.doCheckLoading()}.bind({data:mdl_obj}));break;case "skybox":my_loader=
new THREE.CubeTextureLoader(manager);my_loader.setPath("media/models/skyboxes/"+mdl_obj.name+"/");my_loader.load(["right.jpg","left.jpg","up.jpg","down.jpg","front.jpg","back.jpg"],function(map){if(platform.isMobile){map.magFilter=THREE.NearestFilter;map.minFilter=THREE.NearestMipMapNearestFilter}else map.minFilter=THREE.LinearFilter;lib[this.name]=map;manager.items_to_process--;manager.doCheckLoading()}.bind({name:mdl_obj.name}));break}}};this.doLoadAssets=function(obj){var me=this;images=images||
{};var loader_dummy=document.documentElement.appendChild(document.createElement("div"));loader_dummy.className="loader_dummy";loader_dummy.id="loader_dummy";var loader=new createjs.LoadQueue(false);loader.installPlugin(createjs.Sound);loader.removeEventListener("fileload",me.onFileLoad);loader.removeEventListener("progress",me.onProgress);loader.removeEventListener("complete",me.onComplete);loader.removeEventListener("error",me.onError);this.onFileLoad=function(event){if(event.item.type=="javascript"){var script=
document.createElement("script");script.setAttribute("type","text/javascript");script.setAttribute("src",event.item.src);document.getElementsByTagName("head")[0].appendChild(script)}else if(event.item.type=="image"||event.item.type=="svg")images[event.item.id]=event.result;else if(event.item.type=="text"){var txt=loader_dummy.appendChild(document.createElement("div"));txt.className="general_absolute";txt.style.fontFamily=event.item.id;txt.style.color="white";txt.style.visibility="hidden";txt.innerHTML=
"-"}};this.onError=function(event){trace(event.title+": "+event.data.src);try{assets_retry.manifest.push({src:event.data.src,id:event.data.id})}catch(e$1){}};this.onProgress=function(event){obj.progress=event.loaded};this.onComplete=function(event){obj.progress=1;obj.loaded=true;obj.loadTime=window.performance.now();loader.removeEventListener("fileload",me.onFileLoad);loader.removeEventListener("progress",me.onProgress);loader.removeEventListener("complete",me.onComplete);loader.removeEventListener("error",
me.onError);__utils.doDestroyElement(document.getElementById("loader_dummy"))};loader.addEventListener("fileload",me.onFileLoad);loader.addEventListener("progress",me.onProgress);loader.addEventListener("complete",me.onComplete);loader.addEventListener("error",me.onError);loader.loadManifest(obj.manifest)};this.OLD_doInitFocusManager=function(callback_lose_focus,callback_get_focus){var visProp=me.doGetHiddenProp();if(visProp){var evtname=visProp.replace(/[H|h]idden/,"")+"visibilitychange";document.addEventListener(evtname,
function(evt){if(document[visProp]){__snds.forceMute();if(callback_lose_focus)callback_lose_focus()}else{__snds.unforceMute();if(callback_get_focus)callback_get_focus()}})}};this.doFade=function(elem,to,seconds,callback){var fader=elem;fader.my_opacity=Number(fader.style.opacity);fader.to_opacity=to;fader.step=(to-fader.my_opacity)/(seconds*60);fader.forget=false;fader.doUpdate=function(){if(this.step>0)this.my_opacity=Math.min(this.to_opacity,this.my_opacity+this.step);else this.my_opacity=Math.max(this.to_opacity,
this.my_opacity+this.step);this.style.opacity=this.my_opacity;if(this.my_opacity==this.to_opacity){this.forget=true;if(callback)callback()}};actives.push(fader)};this.doFadeTo=function(elem,callback){var fader=elem;fader.style.display="block";fader.style.opacity=0;fader.mode=1;fader.my_opacity=0;fader.doUpdate=function(){switch(this.mode){case 1:this.my_opacity+=.05;this.style.opacity=Math.min(1,Math.max(0,this.my_opacity));if(this.my_opacity>=1){this.mode=2;if(callback)callback()}break;case 2:this.my_opacity-=
.05;this.style.opacity=Math.max(0,Math.min(1,this.my_opacity));if(this.my_opacity<=0){this.purge=true;this.style.display="none"}break}};actives.push(fader)};this.NewPulse=function(pulse_speed,o){var me=this;var o=o||{};var pulse=o.seed||0;var ease=o.ease||1;var ease_speed=o.ease_speed||1;me.value=0;me.value_normal=0;this.reset=function(o){var pulse=o.seed||0;var ease=o.ease||1;var ease_speed=o.ease_speed||1;var pulse_speed=o.pulse_speed||pulse_speed;me.value=Math.sin(__utils.radFromDeg(pulse));me.value_normal=
.5+me.value*.5};this.update=function(){pulse+=pulse_speed;if(pulse>=360)pulse-=360;me.value=Math.sin(__utils.radFromDeg(pulse))*ease;me.value_normal=.5+me.value*.5*ease;ease=Math.min(1,ease+ease_speed)}};this.dataURItoBlob=function(dataURI){var byteString,mimestring;if(dataURI.split(",")[0].indexOf("base64")!==-1)byteString=atob(dataURI.split(",")[1]);else byteString=decodeURI(dataURI.split(",")[1]);mimestring=dataURI.split(",")[0].split(":")[1].split(";")[0];var content=new Array;for(var i=0;i<byteString.length;i++)content[i]=
byteString.charCodeAt(i);return new Blob([new Uint8Array(content)],{type:mimestring})};this.ordinal_suffix_of=function(i){var j=i%10,k=i%100;if(j==1&&k!=11)return"st";if(j==2&&k!=12)return"nd";if(j==3&&k!=13)return"rd";return"th"};this.doFader=function(){fader.style.display="inline-block";fader.style.opacity=1;var upd=new Object;upd.keep=true;upd.opacity=1.5;upd.update_function=function(){this.opacity-=.1;fader.style.opacity=Math.max(0,Math.min(1,this.opacity));if(this.opacity<=0){this.keep=false;
fader.style.display="none"}};updateQueue.push(upd)};this.doGetBrowser=function(){var ua=navigator.userAgent,tem,M=ua.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i)||[];if(/trident/i.test(M[1])){tem=/\brv[ :]+(\d+)/g.exec(ua)||[];return"IE "+(tem[1]||"")}if(M[1]==="Chrome"){tem=ua.match(/\bOPR\/(\d+)/);if(tem!=null)return"Opera "+tem[1]}M=M[2]?[M[1],M[2]]:[navigator.appName,navigator.appVersion,"-?"];if((tem=ua.match(/version\/(\d+)/i))!=null)M.splice(1,1,tem[1]);return M.join(" ")};
this.getCSSPPI=function(){var div=document.createElement("div");div.style.width="1in";var body=document.getElementsByTagName("body")[0];body.appendChild(div);var ppi=div.offsetWidth;body.removeChild(div);return parseFloat(ppi)};this.getPPI=function(){var div=document.createElement("div");div.style.width="1in";var body=document.getElementsByTagName("body")[0];body.appendChild(div);var devicePixelRatio=window.devicePixelRatio||1;var ppi=div.offsetWidth*devicePixelRatio;body.removeChild(div);return parseFloat(ppi)};
this.getPixelRatio=function(){var ctx=document.createElement("canvas").getContext("2d"),dpr=window.devicePixelRatio||1,bsr=ctx.webkitBackingStorePixelRatio||ctx.mozBackingStorePixelRatio||ctx.msBackingStorePixelRatio||ctx.oBackingStorePixelRatio||ctx.backingStorePixelRatio||1;return dpr/bsr};this.doGetLanguage=function(){var lang;if(navigator&&navigator.userAgent&&(lang=navigator.userAgent.match(/android.*\W(\w\w)-(\w\w)\W/i)))lang=lang[1];if(!lang&&navigator){if(navigator.language)lang=navigator.language;
else if(navigator.browserLanguage)lang=navigator.browserLanguage;else if(navigator.systemLanguage)lang=navigator.systemLanguage;else if(navigator.userLanguage)lang=navigator.userLanguage;lang=lang.substr(0,2)}return lang};this.getQueryString=function(){var query_string={};var query=window.location.search.substring(1);if(query.length<1)if(window.location.toString().split("%3F")[1]!=null)query=window.location.toString().split("%3F")[1];var vars=query.split("&");for(var i=0;i<vars.length;i++){var pair=
vars[i].split("=");if(typeof query_string[pair[0]]==="undefined")query_string[pair[0]]=pair[1];else if(typeof query_string[pair[0]]==="string"){var arr=[query_string[pair[0]],pair[1]];query_string[pair[0]]=arr}else query_string[pair[0]].push(pair[1])}return query_string};this.s4=function(){return Math.floor((1+Math.random())*65536).toString(16).substring(1)};this.guid=function(){return this.s4()+this.s4()+"-"+this.s4()+"-"+this.s4()+"-"+this.s4()+"-"+this.s4()+this.s4()+this.s4()};this.formatDate=
function(date,format,utc){var MMMM=["\x00","January","February","March","April","May","June","July","August","September","October","November","December"];var MMM=["\u0001","Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];var dddd=["\u0002","Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];var ddd=["\u0003","Sun","Mon","Tue","Wed","Thu","Fri","Sat"];function ii(i,len){var s=i+"";len=len||2;while(s.length<len)s="0"+s;return s}var y=utc?date.getUTCFullYear():
date.getFullYear();format=format.replace(/(^|[^\\])yyyy+/g,"$1"+y);format=format.replace(/(^|[^\\])yy/g,"$1"+y.toString().substr(2,2));format=format.replace(/(^|[^\\])y/g,"$1"+y);var M=(utc?date.getUTCMonth():date.getMonth())+1;format=format.replace(/(^|[^\\])MMMM+/g,"$1"+MMMM[0]);format=format.replace(/(^|[^\\])MMM/g,"$1"+MMM[0]);format=format.replace(/(^|[^\\])MM/g,"$1"+ii(M));format=format.replace(/(^|[^\\])M/g,"$1"+M);var d=utc?date.getUTCDate():date.getDate();format=format.replace(/(^|[^\\])dddd+/g,
"$1"+dddd[0]);format=format.replace(/(^|[^\\])ddd/g,"$1"+ddd[0]);format=format.replace(/(^|[^\\])dd/g,"$1"+ii(d));format=format.replace(/(^|[^\\])d/g,"$1"+d);var H=utc?date.getUTCHours():date.getHours();format=format.replace(/(^|[^\\])HH+/g,"$1"+ii(H));format=format.replace(/(^|[^\\])H/g,"$1"+H);var h=H>12?H-12:H==0?12:H;format=format.replace(/(^|[^\\])hh+/g,"$1"+ii(h));format=format.replace(/(^|[^\\])h/g,"$1"+h);var m=utc?date.getUTCMinutes():date.getMinutes();format=format.replace(/(^|[^\\])mm+/g,
"$1"+ii(m));format=format.replace(/(^|[^\\])m/g,"$1"+m);var s=utc?date.getUTCSeconds():date.getSeconds();format=format.replace(/(^|[^\\])ss+/g,"$1"+ii(s));format=format.replace(/(^|[^\\])s/g,"$1"+s);var f=utc?date.getUTCMilliseconds():date.getMilliseconds();format=format.replace(/(^|[^\\])fff+/g,"$1"+ii(f,3));f=Math.round(f/10);format=format.replace(/(^|[^\\])ff/g,"$1"+ii(f));f=Math.round(f/10);format=format.replace(/(^|[^\\])f/g,"$1"+f);var T=H<12?"AM":"PM";format=format.replace(/(^|[^\\])TT+/g,
"$1"+T);format=format.replace(/(^|[^\\])T/g,"$1"+T.charAt(0));var t=T.toLowerCase();format=format.replace(/(^|[^\\])tt+/g,"$1"+t);format=format.replace(/(^|[^\\])t/g,"$1"+t.charAt(0));var tz=-date.getTimezoneOffset();var K=utc||!tz?"Z":tz>0?"+":"-";if(!utc){tz=Math.abs(tz);var tzHrs=Math.floor(tz/60);var tzMin=tz%60;K+=ii(tzHrs)+":"+ii(tzMin)}format=format.replace(/(^|[^\\])K/g,"$1"+K);var day=(utc?date.getUTCDay():date.getDay())+1;format=format.replace(new RegExp(dddd[0],"g"),dddd[day]);format=format.replace(new RegExp(ddd[0],
"g"),ddd[day]);format=format.replace(new RegExp(MMMM[0],"g"),MMMM[M]);format=format.replace(new RegExp(MMM[0],"g"),MMM[M]);format=format.replace(/\\(.)/g,"$1");return format};this.getDayNumber=function(){var minutes=1E3*60;var hours=minutes*60;var days=hours*24;var years=days*365;var base=new Date(2014,0,1);var now=new Date;var t=now.getTime()-base.getTime();var daynum=Math.ceil(t/days);return daynum};this.doGoFullScreen=function(){var el=document.documentElement,rfs=el.requestFullScreen||el.webkitRequestFullScreen||
el.mozRequestFullScreen;rfs.call(el)};this.doCheckFullScreen=function(){trace("doCheckFullScreen");var isInFullScreen=document.fullscreenEnabled||document.mozFullscreenEnabled||document.webkitIsFullScreen?true:false;return isInFullScreen};this.doFullScreenOn=function(){var elem=document.documentElement;if(elem.requestFullscreen)elem.requestFullscreen();else if(elem.msRequestFullscreen)elem.msRequestFullscreen();else if(elem.mozRequestFullScreen)elem.mozRequestFullScreen();else if(elem.webkitRequestFullscreen)elem.webkitRequestFullscreen();
oSTAGE.is_fullscreen=true};this.doFullScreenOff=function(){if(document.exitFullscreen)document.exitFullscreen();else if(document.cancelFullScreen)document.cancelFullScreen();else if(document.mozCancelFullScreen)document.mozCancelFullScreen();else if(document.msExitFullscreen)document.msExitFullscreen();else if(document.webkitExitFullscreen)document.webkitExitFullscreen();oSTAGE.is_fullscreen=false};this.doFadeandDestroy=function(e){var me=e.currentTarget;me.alpha-=.1;if(me.alpha<=0)me.parent.removeChild(me)};
this.doGetPOT=function(n){return Math.pow(2,Math.ceil(Math.log(n)/Math.log(2)))};this.doCache=function(obj,stage_scale){if(!stage_scale)stage_scale=oSTAGE.cache_scale||oSTAGE.scale;obj.regX=obj.regX|0;obj.regY=obj.regY|0;obj.x=obj.x|0;obj.y=obj.y|0;obj.uncache();var bounds=obj.getBounds();var nom_bounds=obj.nominalBounds;var new_bounds={};if(obj.force_bounds){new_bounds.x=obj.force_bounds.x;new_bounds.y=obj.force_bounds.y;new_bounds.width=obj.force_bounds.width;new_bounds.height=obj.force_bounds.height}else if(!obj.force_bounds&&
bounds&&nom_bounds){new_bounds.x=Math.min(nom_bounds.x,bounds.x);new_bounds.y=Math.min(nom_bounds.y,bounds.y);new_bounds.width=Math.max(nom_bounds.width,bounds.width);new_bounds.height=Math.max(nom_bounds.height,bounds.height)}else if(bounds)new_bounds=bounds;else if(nom_bounds)new_bounds=nom_bounds;else return;var need_POT=false;if(obj.stage)if(obj.stage._webGLContext)need_POT=true;if(need_POT)obj.cache(new_bounds.x,new_bounds.y,__utils.doGetPOT(new_bounds.width),__utils.doGetPOT(new_bounds.height),
stage_scale);else obj.cache(new_bounds.x,new_bounds.y,Math.ceil(new_bounds.width),Math.ceil(new_bounds.height),stage_scale)};this.doLerp=function(a,b,c){return a+c*(b-a)};this.doRound=function(value,decimals){return Number(Math.round(value+"e"+decimals)+"e-"+decimals)};this.doGetIntersect=function(x1,y1,x2,y2,x3,y3,x4,y4){if(x1===x2&&y1===y2||x3===x4&&y3===y4)return false;var denominator=(y4-y3)*(x2-x1)-(x4-x3)*(y2-y1);if(denominator===0)return false;var ua=((x4-x3)*(y1-y3)-(y4-y3)*(x1-x3))/denominator;
var ub=((x2-x1)*(y1-y3)-(y2-y1)*(x1-x3))/denominator;if(ua<0||ua>1||ub<0||ub>1)return false;var x=x1+ua*(x2-x1);var y=y1+ua*(y2-y1);return{x:x,y:y}};this.doGetDistance=function(x1,y1,x2,y2){var x=x1-x2;var y=y1-y2;return Math.sqrt(x*x+y*y)};this.doGetAngle=function(cx,cy,ex,ey){var dy=ey-cy;var dx=ex-cx;var theta=Math.atan2(dy,dx);theta*=180/Math.PI;return theta};this.degFromRad=function(p_radInput){var degOutput=180/Math.PI*p_radInput;return degOutput};this.radFromDeg=function(p_degInput){var radOutput=
Math.PI/180*p_degInput;return radOutput};this.getRandomArbitrary=function(min,max){return Math.random()*(max-min)+min};this.getRandomInt=function(min,max){return Math.floor(Math.random()*(max-min+1))+min};this.doLerp=function(start,end,amt){return(1-amt)*start+amt*end};this.doRandomizeArray=function(theArray){var arrayLength=theArray.length;var oldArray=theArray.slice();var newArray=new Array;while(oldArray.length>0){var tId=Math.floor(Math.random()*oldArray.length);var tItem=oldArray[tId];oldArray.splice(tId,
1);newArray.push(tItem)}return newArray};this.doClearTimeouts=function(){for(var i=0;i<timeouts.length;i++)clearTimeout(timeouts[i]);timeouts=[]};this.getPointInBetweenByPerc=function(pointA,pointB,percentage){var dir=pointB.clone().sub(pointA);var len=dir.length();dir=dir.normalize().multiplyScalar(len*percentage);return pointA.clone().add(dir)};this.doText=function(obj,o,params){params=params||{};if(obj.my_origin){obj.x=obj.my_origin.x;obj.y=obj.my_origin.y}else{var bounds=obj.getBounds()||{height:0};
obj.my_origin={x:obj.x,y:obj.y,h:bounds.height,lineHeight:obj.lineHeight,center:obj.y+bounds.height*.5}}if(typeof o!=="object"||o===null)var o={value:o};var msg=o.value||o.txt;for(var s in o)if(s!="value")obj[s]=o[s];if(!msg&&msg!=0&&msg!=""){msg="MISSING TEXT";obj.color="#FF0000"}var font=obj.font;var family_start=font.indexOf("'");var family_end=font.lastIndexOf("'");if(!obj.original_font_family)obj.original_font_family=font.slice(family_start+1,family_end);var font_family=obj.original_font_family;
var size_start=0;var size_end=font.indexOf("px");for(var i=size_end;i>=0;i--)if(font[i]==" "){size_start=i;break}if(!obj.original_font_size)obj.original_font_size=Number(font.slice(size_start,size_end));var font_size=obj.original_font_size;var t_font_weight=font.substring(0,size_start)+font.substring(size_end+2,family_start)+font.substring(family_end,font.length-1);t_font_weight=t_font_weight.split(" ").join("");if(!obj.original_font_weight)obj.original_font_weight=t_font_weight;var font_weight=obj.original_font_weight;
if(obj.fontFamily)font_family=obj.fontFamily;if(obj.fontWeight)font_weight=obj.fontWeight;if(o.fontSize)if(o.fontSize.indexOf("px")!=-1)font_size=o.fontSize.split("px")[0];else if(o.fontSize.indexOf("%")!=-1){var font_percentage=o.fontSize.split("%")[0]*.01;font_size*=font_percentage;obj.lineHeight=obj.my_origin.lineHeight*font_percentage}if(o.lineHeight)if(String(o.lineHeight).indexOf("%")!=-1){var lineheight_percentage=Number(String(o.lineHeight).split("%")[0]*.01);obj.lineHeight=obj.my_origin.lineHeight*
lineheight_percentage}else obj.lineHeight=Number(o.lineHeight);obj.font_size=font_size;if(obj.original_font_size!=obj.font_size)if(params.verticalAlign=="middle")obj.y+=obj.my_origin.lineHeight*.5-obj.lineHeight*.5;else if(params.verticalAlign=="baseline")obj.y+=obj.my_origin.lineHeight-obj.lineHeight;if(font_weight.length>=1)obj.font=font_weight+" "+font_size+"px "+"'"+font_family+"'";else obj.font=font_size+"px "+"'"+font_family+"'";msg=String(msg);var patt=/\<br>/g;msg=msg.replace(patt,"\n");var patt=
/\{\*br\*\}/g;msg=msg.replace(patt,"\n");var patt=/\{\*p\*\}/g;msg=msg.replace(patt,"\n\n");var patt=/\<p>/g;msg=msg.replace(patt,"\n\n");obj.text=msg;var pixel_ratio=__utils.getPixelRatio();var pixel_ratio_inv=1/oSTAGE.pixel_ratio;var metrics=obj.getMetrics();var number_of_lines=metrics.lines.length;var block_height=obj.lineHeight*number_of_lines;var line_offset=font_size-obj.lineHeight;if(!params.verticalAlign||params.verticalAlign=="middle"){obj.textBaseline="middle";obj.y+=font_size-Math.max(0,
number_of_lines-1)*metrics.lineHeight*.5;if(font_family!="Poppins"&&font_family.indexOf("Oswald")==-1)obj.y+=obj.getMetrics().vOffset*.5}else if(params.verticalAlign=="baseline"){obj.textBaseline="baseline";obj.y+=font_size-Math.max(0,number_of_lines-1)*metrics.lineHeight}else if(params.verticalAlign=="bottom"){obj.textBaseline="middle";obj.y+=font_size-Math.max(0,number_of_lines-1)*metrics.lineHeight}obj.x=obj.x|0;obj.y=obj.y|0};this.doSubText=function(str,change_from,change_to){var msg=String(str);
var regex=new RegExp(change_from,"g");msg=msg.replace(regex,change_to);return msg};this.doSubValue=function(msg,value){msg=String(msg);var patt=/\{\*var\*\}/g;msg=msg.replace(patt,value);return msg};this.doSubValue2=function(msg,value){msg=String(msg);var patt=/\{\*var2\*\}/g;msg=msg.replace(patt,value);return msg};this.doHTMLText=function(element,o,append){if(typeof o==="object"&&o!==null);else var o={value:o};var msg=o.value;var computed_style=element.my_computed_style||window.getComputedStyle(element,
...
false;stage.needUpdate=true};this.doUpdateFullScreen=function(){trace("doUpdateFullScreen()");if(__window.doCheckFullScreen()){frame.b_fullscreen.mc_on.visible=true;frame.b_fullscreen.mc_off.visible=false}else{frame.b_fullscreen.mc_on.visible=false;frame.b_fullscreen.mc_off.visible=true}};this.doResizeUpdate=function(){var scale=oSTAGE.scale;canvas.style.top=32*scale+"px";canvas.style.width=64*scale+"px";canvas.style.height=180*scale+"px";canvas.width=64*scale*__utils.getPixelRatio();canvas.height=
180*scale*__utils.getPixelRatio();frame.my_scale=scale;me.my_scale=scale;stage.scale=scale*__utils.getPixelRatio();stage.needUpdate=true};this.doShowPause=function(){frame.b_pause.helper.setEnabled(true);frame.b_pause.visible=true;if(platform.isMobile){frame.b_pause.alpha=1;stage.needUpdate=true}else{createjs.Tween.get(frame.b_pause).to({alpha:1},250);stage.activeTweens.push(frame.b_pause)}};this.doHidePause=function(){frame.b_pause.helper.setEnabled(false);if(platform.isMobile){frame.b_pause.visible=
