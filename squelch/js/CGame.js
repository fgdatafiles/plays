function CGame(oData){
    ...
    this._init = function(){
        ...
        // setup GUI
        _oInterface = new CInterface(formatTime(_iTimeLeft), TIME_SHOW_CARDS);
		_oInstructions = new CInstructions();
        _oNextLevelUI = new CNextLevel();
        _oGameOverUI = new CGameOver();
        _oVictoryUI = new CVictory();
    };

	this.disableInstructions = function(){
		_bInstructions = false;
	}
    ...
    this.nextLevel = function(){
		...
		else if (_iCurrentLevel === 2 || _iCurrentLevel == 3)
		{
			TIME_SHOW_CARDS = oData.show_cards * 2000;
		}
		else if (_iCurrentLevel > 4)
		{
			TIME_SHOW_CARDS = oData.show_cards * 3000;
		}
		_oInterface.giveTimer(TIME_SHOW_CARDS);
