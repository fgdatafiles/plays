...
(function()
{
    ...
    game1.initialize=function()
    {
        ...
        main.selectedItem = Math.floor(Math.random()*8);
        playSounds('start');
        game_over=  false;
        nActualLevel = 1;
       ...
       var  bmp = new  c.Bitmap(main.loadedData.getResult('game1_bgd'));
        ...
        document.getElementById('stage-canvas').style.cursor ='none';
        timer = new Timer();
        timer.x = 604;
        timer.y = 15;
        t.addChild(timer);
        TweenLite.from(timer,2,{y:-100,ease:Elastic.easeOut});
        main.energy =10000;
        ...
        piniata.addEventListener('game_over2',onGameOver2);
        odliczanie();

        var dol = new c.Bitmap(main.loadedData.getResult('down_holder'));
        t.addChild(dol);
        dol.setTransform(762,622);
    ...
    function startGame(){
        ...
        timer.start();
        timer.addEventListener('game_over',onGameOver);
        timer.addEventListener('time_over',onTimeOver);
    }


    game1.destroyAndReset = function(s){

        if(!s){
            actualClassLevel = Game1;
        }else{
            actualClassLevel = s;
        }
    ...
    game1.pauseGame = function(){
        timer.pause();
        piniata.pause();
    }
    game1.unpauseGame = function(){
        timer.unpause();
        piniata.unpause();
    }
