function CInterface(szTimeLeft,_itimeCountdown){
    var _pStartPosExit;
    var _pStartPosAudio;
    var _pStartPosScoreText;
    var _pStartPosTimeText;
	var _pStartPosFullscreen;
	var _bDecreaseTimer = false;
    var _oTimeLeft;
    var _oTimeLeft; 
    var _oScore;
    var _oAudioToggle;
    ...
    //var _oButExit;
    var _oScoreMultText;
	var _oTimerCountdown;
	
	var _oButFullscreen;
    var _fRequestFullScreen = null;
    var _fCancelFullScreen = null;
    
    this._init = function(szTimeLeft,_itimeCountdown){
		
		_pStartPosTimeText = {x:30,y:75};
        _szTimeLeft = TEXT_TIMELEFT + szTimeLeft;
        _oTimeLeft = new createjs.Text(_szTimeLeft, "52px "+FONT_GAME, "Yellow");
        ...
        _oScore = new createjs.Text(_szScore, "52px "+FONT_GAME, "Yellow");
		...
		_oTimerCountdown = new createjs.Text("", "150px "+FONT_GAME, "#fff");
        _oTimerCountdown.textAlign = "center";
        _oTimerCountdown.textBaseline = "alphabetic";
        _oTimerCountdown.x = CANVAS_WIDTH/2;
        _oTimerCountdown.y = CANVAS_HEIGHT/2;
        _oTimerCountdown.shadow = new createjs.Shadow("#000000", 2, 2, 2);
        _oTimerCountdown.scaleX = _oTimerCountdown.scaleY = 1.0;
        _oTimerCountdown.visible = false;
        s_oStage.addChild(_oTimerCountdown);
	...
	this.decreaseTimer = function(){
		...
		_bDecreaseTimer = false;
	}
	
	this.giveTimer = function(_itimeCountdown){
		_iCountdown = (_itimeCountdown + 1000) / 1000;
	}
    ...
    this.update = function(szTimeLeft){
		...
		_oTimerCountdown.text = "" + _iCountdown;
		
		//Decrease the timer every 1 second
		if (_bDecreaseTimer === false)
		{
			setTimeout(this.decreaseTimer, 1000);
			_bDecreaseTimer = true;
		}
		if (_iCountdown > 0)
		{
			this.showCountdown();
