...
var jsembed = function()
{
	...
	var resizePolling = 50; // MS to create timeout around resize script procedures
	...
	*/	  
	...
	function embedScalingScript() 
	{
		...
		doResize();
		setTimeout(doResize, 500); // Ugly hack to work around mobile safari's documentSize delay. [Keith] No longer needed?
	}
	/*
	...
	function scaleSet (pScale)
	{
		...
		if(parameters.scaletype!=SCALETYPE_NONE)
		{
			...
			} else if(parameters.scaletype==SCALETYPE_FILL_TO)
			{
				...
				// Alignment param?
				if(parameters.align) {
					var align = parameters.align.split(" ");
					...
					{
						case "center":
							d.style.left = Math.round((getBrowserWidth()/2) - (tFinalW/2)) + "px";
							break;

						case "right":
							d.style.left = Math.round((getBrowserWidth()) - (tFinalW)) + "px";
							break;
					...
					switch(align[1])
					{
						case "center":
						case "middle":
							d.style.top = Math.round((getBrowserHeight()/2) - (tFinalH/2)) + "px";
							break;

						case "bottom":
							d.style.top = Math.round((getBrowserHeight()) - (tFinalH)) + "px";
							break;
				...
				// Default alignment (center horizontal, top aligned)
				else {
					d.style.left = ((getBrowserWidth()/2) - (tFinalW/2)) + "px";
				}
			...
			// CSS scaling of the canvas element directly (hardware scaling)
			if(canvas && parameters.scalecanvas=="style")
			{
				canvas.style.width = d.style.width;
				canvas.style.height = d.style.height;
			}
		}
	...
	function onWindowResize(event)
	{
		...
		_resizeTimeout = setTimeout(doResize, resizePolling);
	}


	/**
	 * Resize script
	 * Used for dynamic stage resizing.
	 */
	function doResize()
	{
		 ...
		 /**
		  * iOS7 navigation bar issue
		  * https://steveush.wordpress.com/2014/03/13/safari-on-ios7-address-and-bookmark-bars-fixed-position-bug/
		 */
		 if(_touchMoveDisabled)
		 {
		 	clearTimeout(_scaleTimeout);
		 	_scaleTimeout = setTimeout(function() {
		 		// Force redraw
	...
	function embedCustomAPIPre()
	{
		switch(getCustomAPI())
		{
			case API_HAXE_FLAMBE:
				// Previously enforced flambe.js here. Now allows for relocating flambe.js if desired. Flambe.js still necessary.
				// Allowing light straight flambe canvas creation for more control
				if(appJs!="default") embedLib(appJs, true, false, 0);
				break;
	...
	function embedCustomAPIMain(tar, app)
	{
		switch(getCustomAPI())
		{
			...
			case API_HAXE_FLAMBE:
				...
				// Internal flambe HTML5 target embedding
				if(appJs=="default") {
					embedFlambe();
					embedScalingScript();
				} 
				// Use flambe.js to embed the game
				else {
					embedScalingScript();
	...
	/**
	 * Internal version of the flambe.embed() script
	 * @see https://github.com/aduros/flambe/blob/master/command/data/embedder/flambe.js
	 */
	
	function embedFlambe(urls, elementId)
	{
		var url = base+"targets/main-html.js";
		var elementId = targetId;
		...
		if ("getContext" in canvas) {
			...
			// Set canvas size
			if(parameters.scalecanvas=="style")
			{
				canvas.width = widthIdeal;
				canvas.height = heightIdeal;
			}
