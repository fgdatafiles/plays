function CCow(iRow,iCol,iX,iY,oParentContainer, szState){
    ...
    this._init = function(iRow,iCol,iX,iY,oParentContainer,state){
        ...
        for(var i = 0; i<COW_NUM; i++){
            ...
            _aScoreNumShadow.push(new createjs.Text(COW_MESSAGE[i],"60px "+FONT, "#000000"));
            _aScoreNumShadow[i].x = iX+2;
            _aScoreNumShadow[i].y = iY-298;
            ...
            _aScoreNum.push(new createjs.Text(COW_MESSAGE[i],"60px "+FONT, "#ffcc00"));
            _aScoreNum[i].x = iX;
            _aScoreNum[i].y = iY-300;
        ...
        _aScoreNumShadow.push(new createjs.Text(COW_MESSAGE[i],"60px "+FONT, "#000000"));
        _aScoreNumShadow[8].x = iX+2;
        _aScoreNumShadow[8].y = iY-298;
        _aScoreNumShadow[8].textAlign = "center";
        _aScoreNumShadow[8].textBaseline = "alphabetic";
        _aScoreNumShadow[8].lineWidth = 200;
        _aScoreNumShadow[8].visible = false;
        s_oStage.addChild(_aScoreNumShadow[8]);

        _aScoreNum.push(new createjs.Text(COW_MESSAGE[i],"60px "+FONT, "#ffcc00"));
        _aScoreNum[8].x = iX;
        _aScoreNum[8].y = iY-300;
        _aScoreNum[8].textAlign = "center";
        _aScoreNum[8].textBaseline = "alphabetic";
        _aScoreNum[8].lineWidth = 200;
        _aScoreNum[8].visible = false;
        s_oStage.addChild(_aScoreNum[8]);
    ...
    this._hitCell = function(iCow){
        if(_bCellOccupied){
            ...
            _aScoreNum[iCow].visible = true;
            createjs.Tween.get(_aScoreNum[iCow] ).to({y:_aScoreNum[iCow].y-50}, 1500);
            createjs.Tween.get(_aScoreNum[iCow] ).to({alpha:0}, 2000).call(function() {_aScoreNum[iCow].visible = false;
                                                                                                               _aScoreNum[iCow].y+=50;
                                                                                                               _aScoreNum[iCow].alpha = 1;
                                                                                                              });
            _aScoreNumShadow[iCow].visible = true;  
            createjs.Tween.get(_aScoreNumShadow[iCow] ).to({y:_aScoreNumShadow[iCow].y-50}, 1500);
            createjs.Tween.get(_aScoreNumShadow[iCow] ).to({alpha:0}, 2000).call(function() {_aScoreNumShadow[iCow].visible = false;
                                                                                                                           _aScoreNumShadow[iCow].y+=50;
                                                                                                                           _aScoreNumShadow[iCow].alpha = 1;
                                                                                                                          });
            _bCellOccupied = false;  
        }
    };
