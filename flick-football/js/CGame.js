function CGame(oData, iTeamChoosed, bMultiplayer, iTeamTwoChoosed) {
    ...
    this._init = function () {
        ...
        _fTimePenality = LAUNCH_PENALITY_SECOND + 2;
		_iPlayerTwoTeam = iTeamTwoChoosed;
        _oBg = createBitmap(s_oSpriteLibrary.getSprite("bg_game"));
		_oBgOverlay = createBitmap(s_oSpriteLibrary.getSprite("bg_game_foreground"));
		...
		if (_bMultiplayer){
			_aOpponentsTeamVs = this._createMultiplayerTeam();
		}
		else{
		...
		s_oScrollStage.addChild(_oBgOverlay);
    ...
    this._createPlayers = function () {
        ...
        for (var i = 0; i < PLAYERS_ON_FIELD; i++) {
			...
			if(_bMultiplayer){
				_aPlayers.push(_aPlayerUser[i] = this.createPlayer(oSpritePlayers, USER_PLAYERS[i].x, USER_PLAYERS[i].y, iID, 1, true, USER_PLAYERS[i].goal_keeper, (iUserTeam *5) + i, PLAYERS_USER, s_oScrollStage));
				iID++;
				_aPlayers.push(_aPlayerOpponent[i] = this.createPlayer(oSpritePlayers, OPPONENT_PLAYERS[i].x, OPPONENT_PLAYERS[i].y, iID, 1, true, OPPONENT_PLAYERS[i].goal_keeper, (iOpponentTeam * 5) + i, PLAYERS_USER, s_oScrollStage));
				iID++;
			}
			else{
				_aPlayers.push(_aPlayerUser[i] = this.createPlayer(oSpritePlayers, USER_PLAYERS[i].x, USER_PLAYERS[i].y, iID, 1, true, USER_PLAYERS[i].goal_keeper, (iUserTeam *5) + i, PLAYERS_USER, s_oScrollStage));
				iID++;
				_aPlayers.push(_aPlayerOpponent[i] = this.createPlayer(oSpritePlayers, OPPONENT_PLAYERS[i].x, OPPONENT_PLAYERS[i].y, iID, 1, false, OPPONENT_PLAYERS[i].goal_keeper, (iOpponentTeam * 5) + i, PLAYERS_OPPONENT, s_oScrollStage));
				iID++;
			}
	...
	this._createMultiplayerTeam = function () {
        var aTeam = new Array();
        var iID = 0;
        aTeam[iID] = _iPlayerTwoTeam;
        return aTeam;
    };
	...
	this.onRematchMultiplayer = function(){
		s_oMain.gotoTeamChoose(true);
	};
    ...
    this.startMatch = function () {
		if (_bMultiplayer)
		{
			for(var i = 0; i < _aPlayerOpponent.length; i++)
			{
				_aPlayerOpponent[i].turnListenerOff();
				_aPlayerUser[i].turnListenerOn();
			}
		}
    ...
    this.changeOpponenPlayersTeam = function () {
        for (var i = 0; i < _aPlayerOpponent.length; i++) {
            _aPlayerOpponent[i].changeTeam((_aOpponentsTeamVs[_iLevel] * 5) + i);
        }
    };
    ...
    this.finishTime = function () {
        ...
        if (_iGoalPlayer > _iGoalOpponent) {
            ...
            _oInterface.refreshScore(_iTotScore);
            if ((_iLevel === TOT_MATCH - 1) && (_iWinCount >= _iLoseCount)) {
                bEnd = true;
            }
        } else if ((_iGoalPlayer === _iGoalOpponent)){
			bWin = true;
			bDraw = true;
			_iDrawCount ++;
			playSound("goal", 1, false);
            ...
            _iTotScore = oScore.new_score = oScore.new_score + _iLevelScore;
            _oInterface.refreshScore(_iTotScore);
			 if ((_iLevel === TOT_MATCH - 1) && (_iDrawCount >= _iLoseCount)) {
                bEnd = true;
            }
		}
		else {
        ...
        _oInterface.createEndMatchText(_iGoalPlayer, _iGoalOpponent, bWin, oScore, bEnd, _bMultiplayer, bDraw);
    };
        ...
        if (!_bCpuTurn) {
            if (_iNumOfShot === 0) {
				...
				if (_bMultiplayer)
				{
					for(var i = 0; i < _aPlayerOpponent.length; i++)
					{
						_aPlayerOpponent[i].turnListenerOn();
						_aPlayerUser[i].turnListenerOff();
					}
				}
                if (_fTimePenality <= 0) {
					if (_bMultiplayer)
					{
						_oInterface.turnAnim(TEXT_LAUNCH_DELAY_PENALTY_MULTIPLAYER);
					}
					else{
					...
					if (_bMultiplayer){
						_oInterface.turnAnim("Player 2's turn");
					}
					else{
            ...
            if (_iNumOfShot === 0) {
				...
				if (_bMultiplayer) {
					...
					if (_fTimePenality <= 0) {
						_oInterface.turnAnim(TEXT_LAUNCH_DELAY_PENALTY_MULTIPLAYER_2);
					}
					else {
						_oInterface.turnAnim("Player 1's Turn");
					}
    ...
    this.prepareCpuForLaunch = function (oPlayer, bAllPlayerLeftBall, vTargetPlayer, isGoalKeeper) {
        ...
        if (oPlayer.isGoalKeeper()) {
            iPower = CPU_POWER_PLAYER[_iLevel] * 0.5; // 0.5
        }
        if (bAllPlayerLeftBall) {
            ...
            iPower = CPU_POWER_PLAYER[_iLevel] * 0.8; // 0.8
        } else {
            //****************************
    ...
    this.foul = function () {
        ...
        if (!_bFoulCommitted) {
            ...
            if (!_bCpuTurn) {
				...
				if (_bMultiplayer)
				{
					_oInterface.turnAnim(TEXT_FOUL + " player 2's turn");
				}
				else
				{
				...
				if (_bMultiplayer)
				{
					_oInterface.turnAnim(TEXT_FOUL + " player 1's turn");
				}
				else
				{
