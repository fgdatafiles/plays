...
//Full game mode (false = single, parceled-out mini-game). 
var fullGameMode = true;

//If 'fullGameMode = false', this is the individual mini-game ID to use.. If 'fullGameMode = true', this doesn't matter (is ignored).
var miniGameID = "swinging";
    ...
    require(['pixi', 'md'], function(pixi, md){
        ...
        require([
           game_base_path+"minifiedjs/game.min.js"
            ...
            initGame(game_base_path, visualAssets,  audioAssets, ieVersion, retina, deeplink_config_file, fullGameMode, miniGameID);
        });
...
function initGame(base_path, visualAssets, audioAssets, ieVersion, retina, deeplink_config_file, fullGameMode, miniGameID) {
    ...
    if (!window["version"]) {
        gameMarkUp = '<div id="game-holder"></div><div id="parental-approval-page"><div id="nick-form-blank"></div></div><div id="upload-face-page"><div id="file-input"></div></div><div id="scroll-overlay"></div><div id="rotate-overlay"></div>';
    } else {
        gameMarkUp = '<div id="game-debug-info-area">Version: ' + window["version"] + '<br/> Base path: ' + game_base_path + '</div><div id="game-holder"></div><div id="parental-approval-page"><div id="nick-form-blank"></div></div><div id="upload-face-page"><div id="file-input"></div></div><div id="scroll-overlay"></div><div id="rotate-overlay"></div>';
    }
    ...
    game = new Game(base_path, "game-holder", visualAssets, audioAssets, ieVersion, retina, deeplink_config_file, fullGameMode, miniGameID); //Version (displayed on preloader for debugging), base path, string to get main holder div..

    game.addScene(LoadingScene);
    game.addScene(ComeOutSwingingGameplayScene);
    game.addScene(MindTheGapGameplayScene);
    game.addScene(PhotonFusionGameplayScene);
    game.addScene(BaddieBarrageGameplayScene);
...
// Baddie Barrage - "baddie-barrage"

// All games (modes 1 and 2) - "all-games"